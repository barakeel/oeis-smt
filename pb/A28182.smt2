;; sequence(s): A28182
;; terms: 1 31 610 9750 138411 1822821 22826140 275975500 3254034421 37677164811 430475351670 4870239751650 54700558488031 611065572906001 6798955260233200 75422919700042200 834843345187547241
;; small program: loop((loop((loop2((loop((2 + y) * x, 2, x) - x) + ((y * y) * y), y + y, y, 1, 2) + (2 * ((x + x) + x))) + x, y, 1) + (2 * (x + x))) + x, x, 1)
;; fast program: loop((loop(loop2(((2 * (x + x)) + ((y * y) * y)) + x, y + y, y, 1, 2) + (loop((2 + y) * x, 2, x) - x), y, 1) + (2 * ((x + x) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (+ (+ (* 2 (+ x x)) (* (* y y) y)) x))))
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (w6 x y) (- (v7 x) x)))))
