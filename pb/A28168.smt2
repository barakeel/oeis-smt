;; sequence(s): A28168
;; terms: 1 29 536 8086 108927 1369095 16458142 192115232 2198921813 24838420321 278088555108 3094968268938 34308450088459 379311805532507 4186301510180234 46149086679258004 508354507662757665
;; small program: loop((loop((loop(loop(loop((2 + y) * x, 2, x) - x, y, 1) + (2 * ((x + x) + x)), y, 1) + (2 * ((x + x) + x))) + x, y, 1) + (2 * (x + x))) + x, x, 1)
;; fast program: loop((loop(loop((loop(loop2(((2 * (x + x)) + x) + y, 2 * ((y + y) + y), x, 1, 2 + (2 + 2)), 1, y) + x) + (((x + x) + x) * 2), x, 1), 1, y) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (g9 x y) (* 2 (+ (+ y y) y)))))
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (+ (+ (v8 x y) x) (* (+ (+ x x) x) 2)))))
