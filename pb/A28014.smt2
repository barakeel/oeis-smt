;; sequence(s): A28014
;; terms: 1 32 671 11686 183561 2702868 38100595 520688018 6954616493 91281855808 1181868771447 15136545033246 192153235734097 2421636122107052 30334353203856827 378040802156676970
;; small program: loop((loop(loop2((loop((2 + y) * x, 2, x) - x) + y, y + y, y, 1, 2) + loop((2 + y) * x, 2, x), y, 1) + (2 * ((x + x) + x))) + x, x, 1)
;; fast program: loop((loop(loop((loop(loop(x + x, x, loop2(1 + (x * y), y, x, 1, 2 + (2 + 2))), 1, y) + (2 * ((x + x) + x))) + x, x, 1), 1, y) + ((((x + x) * 2) + x) * 2)) + x, x, 1)
(set-logic UFNIA)
