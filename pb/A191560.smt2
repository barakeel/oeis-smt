;; sequence(s): A191560
;; terms: 15625 6103515625 2384185791015625 931322574615478515625 363797880709171295166015625 142108547152020037174224853515625 55511151231257827021181583404541015625 21684043449710088680149056017398834228515625 8470329472543003390683225006796419620513916015625
;; small program: loop((2 * (x + x)) + x, 2 * loop(1 + (x + x), 2, x), 1)
;; fast program: loop((x * x) * x, 1, loop(x * x, 1, loop2(x * y, y, 1 + x, 1, 1 + (2 + 2)))) * loop(x * x, 1, loop2(x * y, y, x, 1, 1 + (2 + 2)))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g0 x) (* 2 (v1 x)))))
(assert (= h0 1))
