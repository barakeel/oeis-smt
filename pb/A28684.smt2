;; sequence(s): A28684
;; terms: 1 462 108002664 12244181663697984 671907296255880840847211520 17845802448787704188422900898268569763840 229407835408995110669430829173445733902175228551055278080
;; small program: loop(loop((x * x) - x, 1, loop(2 * (loop((2 + y) * x, 2, x) - x), y, 1)) * x, x, 1)
;; fast program: loop(loop((x * x) - x, 1, loop2(x * y, y, y, 1, 2 + (2 * (2 + (2 * (2 + 2)))))) * x, x, 1)
(set-logic UFNIA)
