;; sequence(s): A28114
;; terms: 1 27 475 6975 93451 1191447 14779315 180568575 2187814651 26388443367 317513959555 3815510712975 45819341339851 550037207863287 6601706608906195 79228188287078175 950785259085745051
;; small program: loop((loop(loop(loop(2 * ((x + x) + x), x, loop(x + x, x, 2) - 1), 1, y) + (2 * (x + x)), y, 1) + (2 * (x + x))) + x, x, 1)
;; fast program: loop((loop(loop((loop(loop((x + x) + x, x, loop(x + x, x, 2) - 1), 1, y) + x) + x, x, 1) * loop(x + x, x, 1), 1, y) + (2 * (x + x))) + x, x, 1)
(set-logic UFNIA)
