;; sequence(s): A330581
;; terms: 2 3 10 1001 1004006004002 1020191144865623440455270145683555422808365843606721760320033
;; small program: loop(1 + loop2(x * y, y, y, 1, x), x, 2)
;; fast program: loop((loop2(x * y, y, y - 1, 1, x) * (x * x)) + 1, x - 1, loop(if x <= 0 then 1 else (1 + 2), x, 2))
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (* (w3 x y) (* x x)) 1))))
(assert (forall ((x Int)) (= (g2 x) (- x 1))))
(assert (forall ((x Int)) (= (f4 x) (ite (<= x 0) 1 (+ 1 2)))))
