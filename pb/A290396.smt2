;; sequence(s): A290396
;; terms: 13 55 277 1513 8593 49825 292417 1729153 10275073 61254145 365945857 2189371393 13111037953 78565515265 470990340097 2824331231233 16939544543233 101611496669185 609565899227137 3656983075356673
;; small program: (((1 + loop2(x * y, y, x, 2, 1 + 2)) + loop(x + x, x, 1)) * loop(x + x, x, 1 + 2)) + 1
;; fast program: ((loop((x * x) + x, 1, loop(x + x, x, 1)) + loop2(x * y, y, x, 2, 2 + (2 + 2))) * (1 + 2)) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (* (+ (+ 1 (w0 x)) (v1 x)) (v2 x)) 1))))
...
(assert (forall ((x Int)) (= (fast x) (+ (* (+ (v3 x) (w5 x)) (+ 1 2)) 1))))
