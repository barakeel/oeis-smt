;; sequence(s): A21974
;; terms: 1 26 459 6916 96005 1270542 16318303 205487672 2552951769 31419668098 384095637107 4672689817068 56644179720493 684877363365494 8264853578776071 99595171342479904 1198894417410885377
;; small program: loop(loop(loop(1 + (2 * (x + x)), y, 1) + loop((x + x) + x, 2, x), y, 1) + loop((2 + y) * x, 2, x), x, 1)
;; fast program: loop((loop(loop2((2 * (2 * ((x + x) + x))) + y, 1 + (2 * (y + y)), x, 1, 1 + (2 + 2)), 1, y) + (2 * (2 * (x + x)))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (+ (* 2 (* 2 (+ (+ x x) x))) y))))
(assert (forall ((x Int) (y Int)) (= (g7 x y) (+ 1 (* 2 (+ y y))))))
