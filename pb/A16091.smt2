;; sequence(s): A16091
;; terms: 1 39 955 18795 325171 5167659 77353795 1107933915 15345293491 207057429579 2736619356835 35572444360635 456191563128211 5785999811488299 72719715037363075 907085298655532955
;; small program: loop(loop(loop(loop((x + x) + x, y + y, 1) + (2 * ((2 * (x + x)) + x)), y, 1) + (2 * (2 * (x + x))), y, 1) + loop((2 + y) * x, 2, x), x, 1)
;; fast program: loop((loop(loop((loop2((2 * ((x + x) + x)) + (y * y), y + y, y, 1, 2) + (2 * (x + x))) + x, x, 1) * loop(x + x, x, 1), 1, y) + (2 * (2 * (x + x)))) + x, x, 1)
(set-logic UFNIA)
