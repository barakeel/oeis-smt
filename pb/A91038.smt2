;; sequence(s): A91038
;; terms: 1 840 619200 447552000 322444800000 232185139200000 167176286208000000 120367284387840000000 86664487757414400000000 62398436345118720000000000 44926874787659120640000000000
;; small program: loop(loop((x * y) + x, 2 + 2, x), x, loop(1 + (2 * ((x + x) + x)), x, 1))
;; fast program: (loop((x * x) * x, 1, loop(x + x, x, 1)) * loop2(x * y, y, x, 1, (2 * (2 * (2 + 2))) - 1)) * loop(1 + (2 * ((x + x) + x)), x, 1)
(set-logic UFNIA)
