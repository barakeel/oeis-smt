;; sequence(s): A28029
;; terms: 1 22 317 3830 42381 447582 4608877 46813030 471994061 4740248942 47508048237 475624522230 4759030936141 47604501583102 476117036332397 4761534717751430 47617186000380621 476181122450194062
;; small program: loop(((loop(loop(loop((2 * (x + x)) + x, x, loop(x + x, x, 2) - 1), 1, y) + (2 * (x + x)), y, 1) + x) + x) + x, x, 1)
;; fast program: loop(loop(((loop((loop(x + x, x, 2) - 1) * loop2(x * y, y, x, 1, 1 + (2 + 2)), 1, y) + x) + x) + x, y, 1) + ((x + x) * 2), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f7 x) (* (- (v8 x) 1) (w9 x)))))
(assert (= g7 1))
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (v6 x y) (* (+ x x) 2)))))
