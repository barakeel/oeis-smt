;; sequence(s): A261125
;; terms: 1 2 48 1935360 40493130637639680000 10654991354747516157752604498631700563938508800000000000
;; small program: loop2(loop(x * y, y, x), y + y, x, 1, 2)
;; fast program: loop(loop(loop(loop2(x * y, 1 + y, x - 1, 1, 1 + x) * loop(x * y, x, 1), 1, loop(x + x, x, 1)), 1, y) * x, x - 1, 1) * loop2(x * y, y + y, x, 1, 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (v2 x) (w8 x)))))
