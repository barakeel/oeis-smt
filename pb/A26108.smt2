;; sequence(s): A26108
;; terms: 1 28 519 8078 114617 1539720 19981483 253451506 3165132333 39098271212 479245678847 5841627343734 70916047801249 858343285202704 10366285999237011 124990982044693562 1505251989334074965
;; small program: loop((loop((loop(loop((x + x) + x, y + y, 1) + loop((2 + y) * x, 2, x), y, 1) + (2 * (x + x))) + x, y, 1) + x) + x, x, 1)
;; fast program: loop((loop(loop((loop(loop2(1 + (x * y), y, x, 1, 2 + (2 + 2)) * loop(x + x, x, 1), 1, y) + (2 * (x + x))) + x, x, 1), 1, y) + x) + (((x + x) * 2) * 2), x, 1)
(set-logic UFNIA)
