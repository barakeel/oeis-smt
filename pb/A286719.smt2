;; sequence(s): A286719
;; terms: 1 25 395 5075 58086 618870 6289690 61885450 595122671 5629238615 52605474285 487197745125 4481780785756 41018845739260 373968405050180 3399402534376100 30830907772159341 279134548584080805 2523817507756513375 22795663165336810375
;; small program: loop(((loop((loop((loop(1 + loop((x + x) + x, 2, x), y, 1) + (2 * ((x + x) + x))) + x, y, 1) + (2 * (x + x))) + x, y, 1) + x) + x) + x, x, 1)
;; fast program: loop((loop(loop((loop(loop2(((2 * (x + x)) + x) + y, 1 + ((y + y) + y), x, 1, 2 + 2), 1, y) + (2 * ((x + x) + x))) + x, x, 1), 1, y) + x) + (((x * 2) * 2) * 2), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f9 x y) (+ (+ (* 2 (+ x x)) x) y))))
(assert (forall ((x Int) (y Int)) (= (g9 x y) (+ 1 (+ (+ y y) y)))))
