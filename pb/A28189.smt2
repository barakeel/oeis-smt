;; sequence(s): A28189
;; terms: 1 35 782 14254 231459 3493041 50160064 695287148 9389676197 124321003807 1620910078866 20877357884682 266266935091015 3368664326093933 42334402590112388 529044108699525256
;; small program: loop((loop((loop(loop(loop((2 + y) * x, 2, x) - x, y, 1) + loop((2 + y) * x, 2, x), y, 1) + (2 * ((x + x) + x))) + x, y, 1) + (2 * (x + x))) + x, x, 1)
;; fast program: loop((loop(loop((loop(loop2((2 * (2 * ((x + x) + x))) + y, (2 * (y + y)) + y, x, 1, 1 + (2 + 2)), 1, y) + (2 * ((x + x) + x))) + x, x, 1), 1, y) + ((((x + x) * 2) + x) * 2)) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f10 x y) (+ (* 2 (* 2 (+ (+ x x) x))) y))))
