...
(assert (forall ((x Int)) (= (small x) (+ (+ (+ (+ (divf (+ (+ (divf (- (* 2 (+ x x)) (modf (divf x 2) 2)) (+ 1 2)) 1) x) (ite (<= x 0) 1 2)) x) x) x) x))))
(assert (= f0 0))
(assert (forall ((x Int)) (= (g0 x) (- (modf x (+ 2 2)) 2))))
(assert (= h0 2))
(assert (forall ((x Int) (y Int)) (= (u0 x y) (ite (<= x 0) y f0))))
(assert (forall ((x Int)) (= (v0 x) (u0 (g0 x) h0))))
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (+ (divf (+ (+ (divf (+ 1 (+ (v0 x) x)) (+ 1 2)) x) x) (ite (<= x 0) 1 2)) x) x) x) x))))
