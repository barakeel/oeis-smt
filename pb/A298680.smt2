;; sequence(s): A298680
;; terms: 0 0 4 12 56 192 756 2748 10408 38544 144452 537900 2009880 7496160 27985684 104424732 389756936 1454515632 5428480356 20259056268
;; small program: loop((2 * (loop2((2 * (x + x)) - y, x, y - 1, 1, 1) - x)) - 2, x, 0)
;; fast program: loop(((loop2((2 * (x + x)) - y, x, y - 1, 1 + 2, 1) - x) * 2) + 1, x - 1, 1) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (- (* 2 (- (w1 x y) x)) 2))))
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (* (- (w3 x y) x) 2) 1))))
