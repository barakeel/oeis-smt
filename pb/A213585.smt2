;; sequence(s): A213585
;; terms: 1 7 22 54 116 232 443 821 1490 2664 4710 8256 14373 24883 42878 73594 125880 214664 365087 619425
;; small program: loop(loop(loop2(x + y, x, y, 1 + 2, 1 + y) + x, y, x), x, 1) + x
;; fast program: (loop((((loop2(x + y, x, 1 + x, x, 1) - 2) - x) - x) - x, 1, 2 + x) - x) + 1
(set-logic UFNIA)
...
(assert (= i2 (+ 1 2)))
(assert (forall ((x Int) (y Int)) (= (j2 x y) (+ 1 y))))
...
(assert (forall ((x Int)) (= (f3 x) (- (- (- (- (w4 x) 2) x) x) x))))
(assert (= g3 1))
