;; sequence(s): A28102
;; terms: 1 32 659 11122 167577 2347980 31293103 402231494 5031459653 61629924928 742485813147 8826654123066 103796247982129 1209668862433076 13992495704608391 160835129915870638 1838813560663269405 20926823362807299624 237222621983376376435
;; small program: loop(((loop((loop2((2 * ((2 * (x + x)) + x)) + ((y * y) * y), y + y, y, 1, 2) - x) + loop((2 + y) * x, 2, x), y, 1) + x) + x) + x, x, 1)
;; fast program: loop((loop(loop2(((((y * y) * y) + x) + x) + x, y + y, y, 1, 2) + (2 * ((2 * (x + x)) + x)), y, 1) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (+ (+ (+ (* (* y y) y) x) x) x))))
