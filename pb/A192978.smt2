;; sequence(s): A192978
;; terms: 0 1 4 12 29 62 122 227 406 706 1203 2020 3356 5533 9072 14816 24129 39218 63654 103215
;; small program: loop(loop((loop2(x + y, x, y, 1, 2) - 2) + x, y, x), x, 0)
;; fast program: (loop((loop2(x + y, x, x, 2 + 2, 1 + 2) - 1) - ((2 + x) * x), 1, 2 + x) - 2) + x
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (+ (- (w2 x y) 2) x))))
...
(assert (forall ((x Int)) (= (f3 x) (- (- (w4 x) 1) (* (+ 2 x) x)))))
(assert (= g3 1))
