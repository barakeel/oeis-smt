;; sequence(s): A71952
;; terms: 1 40 1092 25664 561104 11807616 243248704 4950550528 100040447232 2013177300992 40412056994816 810023815790592 16221871691714560 324694197936160768 6496965245491888128 129976281056339296256
;; small program: loop(((loop(loop(1 + (2 * ((x + x) + x)), y, 1) + (2 * ((2 * (x + x)) + x)), y, 1) + x) + x) + x, x, 1) * loop(x + x, x, 1)
;; fast program: loop(loop(loop2((2 * ((x + x) + x)) + y, 1 + ((y + y) + y), x, 1, 2 + 2), 1, y) + (2 * ((2 * (x + x)) + x)), x, 1) * loop(x + x, x, 1)
(set-logic UFNIA)
