;; sequence(s): A260404
;; terms: 1 1 2 192 6115295232 15436756676507918107049554083840 18356962141505758798331790171539976807981714702571497465907439808868887035904000000
;; small program: loop(loop(loop(loop(loop(loop(x * y, y, x), y, x), y, x), y, x), y, x), x, 1)
;; fast program: loop(loop(loop(loop(loop(loop((x * y) + x, y, x), y, x), y, 1) * x, y, 1) * x, y, 1) * x, x - 1, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f11 x y) (+ (* x y) x))))
