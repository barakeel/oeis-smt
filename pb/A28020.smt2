;; sequence(s): A28020
;; terms: 1 33 711 12677 203175 3043341 43527247 602201829 8127414999 107618860349 1403846663583 18094236941781 230951316035623 2924174000253357 36776292351134319 459910668538753733
;; small program: loop((loop(loop2((loop((2 + y) * x, 2, x) - x) + ((y * y) * y), y + y, y, 1, 2) + loop((2 + y) * x, 2, x), y, 1) + x) + x, x, 1)
;; fast program: loop((loop(loop2((2 * ((x + x) + x)) + y, 1 + (2 * (y + y)), x, 1, 1 + (2 + 2)) * loop(x + x, x, 1), 1, y) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
(set-logic UFNIA)
