;; sequence(s): A25970
;; terms: 1 24 388 5376 69328 862848 10547776 127767552 1540506880 18530383872 222632510464 2673205936128 32088199647232 385116900458496 4621754370506752 55463163983560704 665570645617475584
;; small program: loop(((loop2((2 * (((x + x) + x) + y)) - 1, y + y, y, 1, 2) + x) + x) + x, x, 1) * loop(x + x, x, 1)
;; fast program: loop(loop2(((2 * (x + y)) - 1) + x, y + y, y, 1, 2) + (2 * ((x + x) + x)), x, 1) * loop(x + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (+ (- (* 2 (+ x y)) 1) x))))
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (+ (w4 x y) (* 2 (+ (+ x x) x))))))
