;; sequence(s): A192974
;; terms: 0 1 4 14 37 84 172 329 600 1058 1821 3080 5144 8513 13996 22902 37349 60764 98692 160105
;; small program: loop(loop(((((loop2(2 + (2 + (x + y)), x, x, 2, 1) - 2) - x) - x) - x) - x, 1, y) + x, x, 0)
;; fast program: (((((((((loop(loop(((loop2(x + y, x, x, 1 + 2, 2) - 2) - x) - x, 1, y) + x, x, 1) + x) * 2) - 1) - x) - x) - x) - 1) - x) - 1) + loop2(x + y, x, x, 1, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (- (- (- (- (- (w2 x) 2) x) x) x) x))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (f4 x) (- (- (- (w5 x) 2) x) x))))
(assert (= g4 1))
...
(assert (forall ((x Int)) (= (fast x) (+ (- (- (- (- (- (- (- (* (+ (v3 x) x) 2) 1) x) x) x) 1) x) 1) (w6 x)))))
