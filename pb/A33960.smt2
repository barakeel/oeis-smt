;; sequence(s): A33960
;; terms: 3 11 27 56 106 190 329 557 929 1534 2516 4108 6687 10863 17623 28564 46270 74922 121285 196305
;; small program: loop(loop(loop2(x + y, x, y, 2, 1) + x, y, x), 1 + x, 0)
;; fast program: loop((loop2(x + y, x, x, 2 * (2 + 2), 1 + (2 + 2)) - 2) - (2 * (2 * (2 + x))), 1, 2 + x) - x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (- (- (w4 x) 2) (* 2 (* 2 (+ 2 x)))))))
(assert (= g3 1))
