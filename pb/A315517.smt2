...
(assert (forall ((x Int)) (= (f1 x) (modf (modf (+ 1 (v2 x)) (+ 1 2)) 2))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (f3 x) (+ (+ (modf (+ (+ (divf (+ 1 (* 2 (+ (divf (+ 1 x) (+ 1 2)) x))) (+ 1 2)) 1) x) 2) 1) x))))
(assert (forall ((x Int)) (= (g3 x) x)))
(assert (= h3 2))
(assert (forall ((x Int) (y Int)) (= (u3 x y) (ite (<= x 0) y (f3 (u3 (- x 1) y))))))
(assert (forall ((x Int)) (= (v3 x) (u3 (g3 x) h3))))
(assert (forall ((x Int)) (= (fast x) (+ (+ (divf (+ (+ (* (+ (+ (+ (- (v3 x) 1) x) x) x) 2) 1) x) (+ 1 2)) x) x))))
