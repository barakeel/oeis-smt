;; sequence(s): A61099
;; terms: 1 64 100 289 361 676 784 1225 1369 1936 2116 2809 3025 3844 4096 5041 5329 6400 6724 7921
;; small program: loop(x * x, 1, loop(loop((x + x) + x, 2, y) - x, x, 1))
;; fast program: loop(x * x, 1, loop(((((2 * 2) * 2) * y) - x) + y, x, 1))
(set-logic UFNIA)
...
(assert (= g3 1))
(assert (forall ((x Int) (y Int)) (= (f4 x y) (+ (- (* (* (* 2 2) 2) y) x) y))))
