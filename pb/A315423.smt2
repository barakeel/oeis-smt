...
(assert (forall ((x Int)) (= (f2 x) (modf (* x x) (- (* 2 (* 2 (+ 2 2))) 1)))))
(assert (= g2 2))
...
(assert (forall ((x Int)) (= (fast x) (+ (divf (+ (+ (* (+ (divf (+ (+ (* (* (+ (+ (+ (+ (+ (divf (- (ite (<= x 0) 1 x) 1) (+ 1 (+ 2 (+ 2 2)))) x) 1) (divf x (+ 1 (+ 2 (+ 2 2))))) x) x) (ite (<= x 0) 1 2)) 2) 1) x) (+ 1 2)) x) 2) 1) x) (+ 1 2)) x))))
