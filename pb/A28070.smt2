;; sequence(s): A28070
;; terms: 1 29 550 8690 124531 1684079 21954280 279351380 3496283461 43256856929 530819707210 6475595107670 78659578436791 952489924910579 11507069203563340 138779617656909560 1671608878215416521
;; small program: loop(((loop((loop(loop((x + x) + x, y + y, 1) + loop((2 + y) * x, 2, x), y, 1) + (2 * (x + x))) + x, y, 1) + x) + x) + x, x, 1)
;; fast program: loop((loop(loop2((2 * (x + x)) + y, 1 + ((y + y) + y), x, 1, 2 + 2) * loop2(x * y, y, x, 1, 1 + 2), 1, y) + (2 * (x + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (+ (* 2 (+ x x)) y))))
(assert (forall ((x Int) (y Int)) (= (g7 x y) (+ 1 (+ (+ y y) y)))))
