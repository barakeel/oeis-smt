;; sequence(s): A13809
;; terms: 5832 612220032 64268410079232 6746640616477458432 708235345355337676357632 74347713614021927913318776832 7804725584345565904628551916716032 819308872942260126404286866009182175232 86007768245986699029416418046179908027154432
;; small program: loop(2 * loop((x + x) + x, 2, x), loop(1 + (x + x), 2, x), 1)
;; fast program: (loop(x * x, 1, loop2(x * y, y, x, 1, 2 + (2 * (2 * (2 + 2))))) * loop((x * x) * x, 1, 2 + (2 * (2 * (2 + 2))))) * loop(x * x, 1, loop2(x * y, y, x, 1, 2 + (2 * (2 * (2 + 2)))))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (* (v3 x) v5) (v6 x)))))
