;; sequence(s): A28124
;; terms: 1 28 503 7406 97545 1198344 14051227 159447442 1766581949 19223504300 206298840111 2189789938518 23040266816113 240689810119696 2499511822660355 25828790012697434 265791691961403237
;; small program: loop(loop(loop(loop((2 * (x + x)) + x, x, loop(x + x, x, 2) - 1), 1, y) + (2 * (x + x)), y, 1) + loop((x + x) + x, 2, x), x, 1)
;; fast program: loop((loop(loop(loop((loop(x + x, x, 2) - 1) * loop2(x * y, y, x, 1, 1 + (2 + 2)), 1, y) + (2 * (x + x)), x, 1), 1, y) + x) + (((x + x) * 2) * 2), x, 1)
(set-logic UFNIA)
