;; sequence(s): A1553
;; terms: 6 21 91 441 2275 12201 67171 376761 2142595 12313161 71340451 415998681 2438235715 14350108521 84740914531 501790686201 2978035877635 17706908038281 105443761093411 628709267031321
;; small program: ((loop((x + x) + x, x, 1) + loop(x + x, x, 1)) * (1 + loop(x + x, x, 1))) + (1 + loop((2 * (x + x)) + x, x, 1))
;; fast program: ((1 + loop((x * x) + x, 1, loop(x + x, x, 1))) + loop2(x * y, y, x, 1 + loop(x + x, x, 1), 1 + 2)) + loop2(x * y, y, x, 1, 1 + (2 + 2))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (+ (* (+ (v0 x) (v1 x)) (+ 1 (v2 x))) (+ 1 (v3 x))))))
...
(assert (forall ((x Int)) (= (i6 x) (+ 1 (v7 x)))))
(assert (= j6 (+ 1 2)))
