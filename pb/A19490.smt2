;; sequence(s): A19490
;; terms: 1 22 340 4600 58576 724192 8822080 106672000 1284971776 15449370112 185571742720 2227940915200 26741787774976 320940501164032 3851520569589760 46219655242547200 554644317650354176
;; small program: loop(loop(loop(2 * ((x + x) + x), x, loop(x + x, x, 2) - 1), 1, y) + (2 * (x + x)), x, 1)
;; fast program: (loop((x * x) - x, 1, loop(x + x, x, 1)) * loop2(x * y, y, x, 1 + 2, 1 + 2)) + loop2(x * y, y, x, 1, 2 + 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (* (v4 x) (w6 x)) (w7 x)))))
