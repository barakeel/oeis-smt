;; sequence(s): A213586
;; terms: 1 6 20 51 112 224 421 758 1324 2263 3808 6336 10457 17158 28036 45675 74256 120544 195485 316790
;; small program: loop(loop(loop2(2 + (x + y), 2 + x, y, 1, 1) + x, y, x), x, 1) + x
;; fast program: (loop(loop((((loop2(x + y, x, x, 1 + (2 * (2 + (2 + 2))), 2 * (2 + 2)) - 2) - (2 * (2 + (2 + x)))) - x) - x, 1, y) + x, x, 1) - x) - x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (- (- (- (- (w5 x) 2) (* 2 (+ 2 (+ 2 x)))) x) x))))
(assert (= g4 1))
