;; sequence(s): A26241
;; terms: 1 29 557 8977 131685 1825017 24374173 317359889 4057667669 51188756905 639248806989 7920904304001 97550444747653 1195603302467993 14597229457092605 177666702622486513
;; small program: loop((loop(loop(loop((2 * (x + x)) + x, x, loop(x + x, x, 2) - 1), 1, y) + loop((2 + y) * x, 2, x), y, 1) + x) + x, x, 1)
;; fast program: loop((loop(loop2((2 * ((x + x) + x)) + y, 1 + ((2 * (y + y)) + y), x, 1, 2 + (2 + 2)) * loop(x + x, x, 1), 1, y) + (2 * (x + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (g8 x y) (+ 1 (+ (* 2 (+ y y)) y)))))
