;; sequence(s): A213770
;; terms: 1 7 23 58 126 250 467 837 1457 2484 4172 6932 11429 18739 30603 49838 81002 131470 213175 345425
;; small program: loop(loop(loop2(2 + (2 + (x + y)), x, y, 1, 0) + x, y, x), x, 1) + x
;; fast program: loop(loop((((loop2(x + y, x, x, 1 + (2 + 2), 2 + 2) - 1) - (2 * (2 + x))) - x) - x, 1, 2 + y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (- (- (- (- (w5 x) 1) (* 2 (+ 2 x))) x) x))))
(assert (= g4 1))
