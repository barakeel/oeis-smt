;; sequence(s): A193004
;; terms: 1 1 9 29 75 165 331 623 1123 1963 3357 5651 9405 15525 25477 41633 67831 110281 179031 290339
;; small program: loop((loop((((((loop2(x + y, x, x, 1 + (2 + (2 + 2)), 2) - 2) - 2) - 2) - x) - x) - x, 1, y) * 2) + x, x, 1)
;; fast program: (loop(loop((((((loop2(x + y, x, x, 1 + (2 + (2 + 2)), 2) - 2) - 2) - 2) - x) - x) - x, 1, y) + x, x, 0) * 2) + 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (- (- (- (- (- (- (w2 x) 2) 2) 2) x) x) x))))
(assert (= g1 1))
...
(assert (forall ((x Int)) (= (f4 x) (- (- (- (- (- (- (w5 x) 2) 2) 2) x) x) x))))
(assert (= g4 1))
