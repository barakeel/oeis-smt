;; sequence(s): A192312
;; terms: 0 4 11 31 70 150 302 588 1113 2065 3770 6796 12124 21444 37655 65715 114078 197122 339242 581740
;; small program: loop(loop(loop2(x + y, x, x, 0, 1 + ((x + x) + x)), 1, y) + x, x, 0)
;; fast program: loop(loop((1 + ((x + x) + x)) * (if x <= 0 then 0 else loop2(x + y, x, x - 2, 1, 1)), 1, y) + x, x, 0)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (* (+ 1 (+ (+ x x) x)) (ite (<= x 0) 0 (w5 x))))))
(assert (= g4 1))
