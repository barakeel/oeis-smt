;; sequence(s): A28022
;; terms: 1 33 709 12573 199957 2966205 41944933 572993421 7627809013 99550699677 1279261319557 16238012145069 204088032533269 2544643371529149 31521003288514981 388369584933658317
;; small program: loop((loop(loop(loop((x + x) + x, y + y, 1) + (2 * ((2 * (x + x)) + x)), y, 1) + loop((2 + y) * x, 2, x), y, 1) + x) + x, x, 1)
;; fast program: loop((loop(loop2((2 * ((x + x) + x)) + y, 1 + ((2 * (y + y)) + y), x, 1, 2 + (2 + 2)) * loop(x + x, x, 1), 1, y) + (2 * (2 * (x + x)))) + x, x, 1)
(set-logic UFNIA)
