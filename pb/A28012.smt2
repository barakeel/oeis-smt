;; sequence(s): A28012
;; terms: 1 30 587 9504 138369 1883826 24504439 308525268 3791612957 45742587462 543912376371 6393243539592 74447130731065 860268600608538 9877405744657583 112802621156970876 1282386938486825493
;; small program: loop((loop(loop2((loop((2 + y) * x, 2, x) - x) + y, y + y, y, 1, 2) + (2 * ((2 * (x + x)) + x)), y, 1) + (2 * ((x + x) + x))) + x, x, 1)
;; fast program: loop((loop(loop((loop(loop2(1 + (x * y), y, x, 1, 1 + (2 + 2)) * loop(x + x, x, 1), 1, y) + (2 * ((x + x) + x))) + x, x, 1), 1, y) + ((((x + x) * 2) + x) * 2)) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f7 x) (* (w8 x) (v9 x)))))
(assert (= g7 1))
