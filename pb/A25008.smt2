;; sequence(s): A25008
;; terms: 1 32 677 11932 189753 2826744 40267309 555414644 7478425985 98842456336 1287451399221 16573995013836 211336837165897 2673608205348008 33601552555975613 419959636392460708
;; small program: loop((loop(loop(1 + (2 * (2 * (x + x))), y, 1) + loop((2 + y) * x, 2, x), y, 1) - x) + loop((2 + y) * x, 2, x), x, 1)
;; fast program: loop((loop(loop(loop2(1 + (x * y), y, y, 1, 2 * (2 + (2 + 2))) + (((x + x) * 2) * 2), x, 1), 1, y) + x) + ((((x + x) * 2) + x) * 2), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (+ (w8 x y) (* (* (+ x x) 2) 2)))))
