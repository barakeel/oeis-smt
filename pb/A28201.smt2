;; sequence(s): A28201
;; terms: 1 31 605 9515 131901 1684011 20303725 234831355 2632942301 28828462091 309891463245 3283433875995 34394321024701 357022927273771 3679144385477165 37693176577349435 384361331408659101
;; small program: loop((loop(loop2((2 * ((x + x) + x)) + ((y * y) * y), y + y, y, 1, 2) + (2 * ((2 * (x + x)) + x)), y, 1) + (2 * ((x + x) + x))) + x, x, 1)
;; fast program: loop((loop(loop((loop2((((y * y) + x) + x) + x, y + y, y, 1, 2) + (2 * (x + x))) + x, x, 1) * loop(x + x, x, 1), 1, y) + (2 * ((x + x) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (+ (w6 x y) (* 2 (+ x x))) x))))
...
(assert (forall ((x Int)) (= (f4 x) (* (v5 x) (v7 x)))))
(assert (= g4 1))
