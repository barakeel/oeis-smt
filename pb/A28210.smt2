;; sequence(s): A28210
;; terms: 1 33 685 11445 168301 2275413 28994365 353654565 4172013901 47945991093 539606148445 5970648969285 65146124616301 702577801647573 7503184109388925 79467822863509605 835719501138799501
;; small program: loop(loop(loop(loop((x + x) + x, y + y, 1) + (2 * ((x + x) + x)), y, 1) + (2 * ((2 * (x + x)) + x)), y, 1) + (2 * (2 * (x + x))), x, 1)
;; fast program: loop((loop(loop((loop2((((y * y) + x) + x) + x, y + y, y, 1, 2) + (2 * (x + x))) + x, x, 1) * loop(x + x, x, 1), 1, y) + (2 * (2 * (x + x)))) + x, x, 1)
(set-logic UFNIA)
