;; sequence(s): A28190
;; terms: 1 32 647 10570 152481 2028012 25481947 307098110 3585577061 40841792992 456139166847 5013707912850 54390770364841 583654905773972 6205991828373347 65478677349042790 686305950702955821
;; small program: loop(loop(loop(loop((2 * (x + x)) + x, x, loop(x + x, x, 2) - 1), 1, y) + (2 * (2 * (x + x))), y, 1) + loop((x + x) + x, 2, x), x, 1)
;; fast program: loop((loop(loop(loop((2 * (x + x)) + x, x, loop(x + x, x, 2) - 1), 1, y) + (2 * (2 * (x + x))), y, 1) + (2 * (2 * (x + x)))) + x, x, 1)
(set-logic UFNIA)
