;; sequence(s): A341330
;; terms: 1 ~7 74 ~950 14675 ~265261 5499540 ~128718188 3358066213 ~96641933787 3041786442934 ~103951418936138 3833424966763151 ~151734670591049073 6416673685121841552 ~288731231494230984304 13774353220573494006705 ~694460992134764182350927
;; small program: loop(loop(loop2(0 - ((x * y) + x), y, 2 + x, 1, x), 1, y) + x, x, 1)
;; fast program: loop(loop((loop2((0 - y) * x, y, x, 1, 1 + x) * (1 + x)) * (1 + x), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (- 0 (+ (* x y) x)))))
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (* (- 0 y) x))))
...
(assert (forall ((x Int)) (= (f4 x) (* (* (w5 x) (+ 1 x)) (+ 1 x)))))
(assert (= g4 1))
