;; sequence(s): A79682
;; terms: 1 4 4096 590295810358705651712
;; small program: loop(x + x, loop((x * x) - 1, 2, x), 1) * loop2(x * y, y, x, 1, 2 + 2)
;; fast program: loop(loop(loop2(loop(loop2((2 * (x * x)) * y, y, x, 1, x), 1, y) * x, y + y, x, 1, 1), 1, y) * x, x, 1) * loop2(x * y, y, x, 1, loop(x + x, x, 1))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (v3 x) (w8 x)))))
