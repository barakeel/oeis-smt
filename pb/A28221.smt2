;; sequence(s): A28221
;; terms: 1 36 817 14964 241969 3609732 50945329 691013268 9103047217 117325825188 1487325837361 18617531653812 230789841098545 2839613067896004 34737008397752113 423047207066166996
;; small program: loop((loop(loop(loop((x + x) + x, y + y, 1) + (2 * (2 * (x + x))), y, 1) + loop((2 + y) * x, 2, x), y, 1) + (2 * ((x + x) + x))) + x, x, 1)
;; fast program: loop((loop(loop((loop(loop2(((x + x) + x) + y, y + y, x, 1, 2) * loop(x * x, 1, loop(x + x, x, 1)), 1, y) + (2 * ((x + x) + x))) + x, x, 1), 1, y) + x) + (((x + x) * 2) * 2), x, 1)
(set-logic UFNIA)
