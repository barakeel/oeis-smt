;; sequence(s): A21254
;; terms: 1 22 341 4646 59661 743358 9112405 110693878 1337742173 16118816558 193887174117 2329875721446 27981116089837 335931645121822 4032287505801077 48395204420052950 580796733493846653
;; small program: loop(loop2(((2 * (((x + x) + x) + y)) - 1) + x, y + y, y, 1, 2) + loop((2 + y) * x, 2, x), x, 1)
;; fast program: loop((loop2((2 * ((2 * ((x + x) + x)) + y)) - 1, y + y, y, 1, 2) + (2 * ((x + x) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (- (* 2 (+ (* 2 (+ (+ x x) x)) y)) 1))))
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (+ (+ (w4 x y) (* 2 (+ (+ x x) x))) x))))
