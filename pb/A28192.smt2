;; sequence(s): A28192
;; terms: 1 34 735 12920 201821 2925054 40302475 535908340 6945496041 88331974874 1107704935415 13744775469360 169190996868661 2070039164549494 25209912745227555 305937712008039980
;; small program: loop((loop(loop(loop((x + x) + x, y + y, 1) + (2 * (2 * (x + x))), y, 1) + loop((2 + y) * x, 2, x), y, 1) + (2 * (x + x))) + x, x, 1)
;; fast program: loop((loop(loop((loop(loop2(((x + x) + x) + y, y + y, x, 1, 2) * loop(x * x, 1, loop(x + x, x, 1)), 1, y) + (2 * (x + x))) + x, x, 1), 1, y) + x) + (((x + x) * 2) * 2), x, 1)
(set-logic UFNIA)
