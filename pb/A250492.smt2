;; sequence(s): A250492
;; terms: 0 1 10610209857723
;; small program: loop(loop2(x + y, x, (x * x) * x, 0, 1), 1, x * x)
;; fast program: loop(loop2(x + y, x, ((x * x) * x) - 2, 1, 1), x - 1, x * x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (h3 x) (- (* (* x x) x) 2))))
(assert (= i3 1))
(assert (= j3 1))
