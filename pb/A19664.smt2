;; sequence(s): A19664
;; terms: 1 21 301 3669 40957 433125 4418317 43942773 428973853 4128937989 39306876973 370937567637 3475860284989 32382187083813 300235508341069 2772487245505461 25515330868003165 234141560259529797
;; small program: loop(loop2(((y - 1) + y) * (y * y), y + y, y, 1, 2) + loop((x + x) + x, 2, x), x, 1)
;; fast program: loop((loop(loop(((x - 1) + x) * (x * x), 1, loop(x + x, x, 1)), 1, y) + x) + (((x + x) * 2) * 2), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (* (+ (- y 1) y) (* y y)))))
