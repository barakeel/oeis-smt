;; sequence(s): A28194
;; terms: 1 35 779 14095 226491 3373575 47712739 650313455 8625455531 112077065815 1433487819699 18110309247615 226591110120571 2813231293937255 34711873203654659 426167229337538575
;; small program: loop(loop(loop(loop((2 * (x + x)) + x, x, loop(x + x, x, 2) - 1), 1, y) + (2 * (2 * (x + x))), y, 1) + loop((2 + y) * x, 2, x), x, 1)
;; fast program: loop(loop(loop(loop(loop((2 * (x + x)) + x, x, loop(x + x, x, 2) - 1), 1, y) + (2 * (2 * (x + x))), x, 1), 1, y) + ((((x + x) + x) * 2) * 2), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f8 x y) (+ (v9 x y) (* 2 (* 2 (+ x x)))))))
