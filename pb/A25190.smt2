;; sequence(s): A25190
;; terms: 1 34 761 14126 235557 3664290 54296257 776238742 10797496253 146990010986 1966611858393 25940502403998 338149901095189 4364412518972722 55856812102918769 709715336973843494
;; small program: loop((loop(loop(1 + (2 * ((2 * (x + x)) + x)), y, 1) + loop((2 + y) * x, 2, x), y, 1) - x) + loop((2 + y) * x, 2, x), x, 1)
;; fast program: loop((loop(loop(loop2(1 + (x * y), y, y, 1, 2 + (2 * (2 + 2))) + ((((x + x) + x) * 2) * 2), x, 1), 1, y) + ((((x + x) * 2) + x) * 2)) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (+ (w8 x y) (* (* (+ (+ x x) x) 2) 2)))))
