;; sequence(s): A188556
;; terms: 6 11 20 36 64 112 192 321 522 825 1268 1898 2772 3958 5536 7599 10254 13623 17844 23072
;; small program: ((loop(loop(loop(2 + loop(x + y, y - 2, x), y, x), y, 2) + x, x, 2) + 2) + 2) + x
;; fast program: ((((loop(loop((loop(loop(x + y, y, x), y - 2, x) + y) + y, y, x), x, 2) + 2) + 2) + x) + x) + x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (+ (+ (v4 x) 2) 2) x) x) x))))
