;; sequence(s): A28056
;; terms: 1 24 373 4800 55861 613224 6497893 67364880 688931221 6985860024 70462212613 708365604960 7106726476981 71208736700424 712951450202533 7134775273637040 71379698771197141 713990571075854424
;; small program: loop(((loop(loop(loop((2 * (x + x)) + x, x, loop(x + x, x, 2) - 1), 1, y) + (2 * ((x + x) + x)), y, 1) + x) + x) + x, x, 1)
;; fast program: loop(loop((loop(loop((x + x) + x, x, loop(x + x, x, 2) - 1), 1, y) + (2 * (x + x))) + x, y, 1) + (2 * ((2 * (x + x)) + x)), x, 1)
(set-logic UFNIA)
