;; sequence(s): A144044
;; terms: 1 4 196 2304 55696 906304 18181696 325730304 6199357696 114211554304 2141141533696 39766241378304 742275639709696 13817991215202304 257604225193885696 4798702283133026304 89428432275804061696
;; small program: loop2(loop2(((2 * ((2 * (y + y)) + y)) - x) - x, x, y, 1, 0) * x, y, 2, 1, x)
;; fast program: loop2(loop(if x <= 0 then 1 else loop2(((2 * ((2 * (y + y)) + y)) + x) + x, x, x - 1, 2, 1), 1, y) * x, y, 2, 1, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (- (- (* 2 (+ (* 2 (+ y y)) y)) x) x))))
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (+ (+ (* 2 (+ (* 2 (+ y y)) y)) x) x))))
...
(assert (forall ((x Int)) (= (f3 x) (ite (<= x 0) 1 (w4 x)))))
(assert (= g3 1))
