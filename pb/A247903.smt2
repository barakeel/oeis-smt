;; sequence(s): A247903
;; terms: 1 5 13 29 53 93 149 237 357 541 789 1165 1669 2429 3445 4973 7013 10077 14165 20301
;; small program: loop(loop(loop2(y + y, x, y, 2, 2) + x, y, x), x, 1)
;; fast program: (((((((loop2(2 * (2 + y), x, x, 1 + 2, 1) - 2) - x) * 2) - 1) - x) - x) * 2) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (* 2 (+ 2 y)))))
...
(assert (forall ((x Int)) (= (fast x) (- (* (- (- (- (* (- (- (w3 x) 2) x) 2) 1) x) x) 2) 1))))
