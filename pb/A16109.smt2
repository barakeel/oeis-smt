;; sequence(s): A16109
;; terms: 1 34 725 12410 186501 2571114 33339685 413066170 4941549701 57504755594 654463491045 7314256515930 80522026412101 875355238834474 9415203971344805 100355146006589690
;; small program: loop((loop(loop(loop((x + x) + x, y + y, 1) + (2 * ((2 * (x + x)) + x)), y, 1) + (2 * (2 * (x + x))), y, 1) + (2 * ((x + x) + x))) + x, x, 1)
;; fast program: loop((loop(loop(loop(loop2((2 * (2 * (x + x))) + (y * y), (y + y) + y, x, 1, 1 + 2), 1, y) + (2 * ((2 * (x + x)) + x)), x, 1), 1, y) + (2 * ((x + x) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f8 x y) (+ (* 2 (* 2 (+ x x))) (* y y)))))
