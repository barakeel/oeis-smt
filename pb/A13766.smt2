;; sequence(s): A13766
;; terms: 20 160000 1280000000 10240000000000 81920000000000000 655360000000000000000 5242880000000000000000000 41943040000000000000000000000 335544320000000000000000000000000 2684354560000000000000000000000000000
;; small program: loop(loop(2 * (2 + x), 2, 2) * x, 1 + ((x + x) + x), 1)
;; fast program: ((2 * loop((x * x) * x, 1, loop(x + x, x, 1))) * loop2(x * y, y, 1 + x, 1, 2 + (2 * (2 + 2)))) * loop(x * x, 1, loop2(x * y, y, x, 1, 2 + (2 * (2 + 2))))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (* (* 2 (v2 x)) (w4 x)) (v5 x)))))
