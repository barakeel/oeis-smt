;; sequence(s): A28187
;; terms: 1 33 692 11790 178311 2498643 33230002 425666400 5302894421 64673833653 775796369712 9184400635410 107582646828931 1249287573291063 14403306409895822 165064341717134820 1882103404758557841 21367906210495206873 241700345807807504332
;; small program: loop((loop(loop(loop((2 * (x + x)) + x, x, loop(x + x, x, 2) - 1), 1, y) + (loop((2 + y) * x, 2, x) - x), y, 1) + (2 * ((x + x) + x))) + x, x, 1)
;; fast program: loop((loop(loop((loop((loop(x + x, x, 2) - 1) * loop2(x * y, y, x, 1, 1 + (2 + 2)), 1, y) + x) + ((((x * 2) * 2) + x) * 2), x, 1), 1, y) + (((x + x) + x) * 2)) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f8 x y) (+ (+ (v9 x y) x) (* (+ (* (* x 2) 2) x) 2)))))
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (+ (+ (v7 x y) (* (+ (+ x x) x) 2)) x))))
