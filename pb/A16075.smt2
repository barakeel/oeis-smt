;; sequence(s): A16075
;; terms: 1 38 905 17290 289821 4453638 64331905 887339330 11810819141 152832918238 1933092302505 23997027406170 293289532268461 3537885908902838 42204462297434705 498697803478957810 5844588402226277781 68011678300853991438 786547256602640400505
;; small program: loop((loop(loop(loop((x + x) + x, y + y, 1) + (2 * ((2 * (x + x)) + x)), y, 1) + (2 * (2 * (x + x))), y, 1) - x) + loop((2 + y) * x, 2, x), x, 1)
;; fast program: loop((loop(loop(((loop(loop2((2 * (2 * (x + x))) + (y * y), (y + y) + y, x, 1, 1 + 2), 1, y) + x) + (((x * 2) * 2) * 2)) + x, x, 1), 1, y) + ((((x + x) * 2) + x) * 2)) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (- (v1 x y) x) (v4 x)))))
(assert (forall ((x Int)) (= (g0 x) x)))
(assert (= h0 1))
(assert (forall ((x Int) (y Int)) (= (u0 x y) (ite (<= x 0) y (f0 (u0 (- x 1) y) x)))))
(assert (forall ((x Int)) (= (v0 x) (u0 (g0 x) h0))))
(assert (forall ((x Int)) (= (small x) (v0 x))))
(assert (forall ((x Int) (y Int)) (= (f9 x y) (+ (* 2 (* 2 (+ x x))) (* y y)))))
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (+ (+ (+ (v8 x y) x) (* (* (* x 2) 2) 2)) x))))
