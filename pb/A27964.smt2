;; sequence(s): A27964
;; terms: 1 7 26 73 174 373 743 1404 2552 4506 7784 13226 22193 36889 60882 99947 163430 266455 433495 704150
;; small program: loop(loop(loop(loop(loop2(y, x + y, y, 2, 1) + x, y, x), y, x), 1 + y, x), x, 1)
;; fast program: loop(loop(loop(loop((((loop2(x + y, x, x, 2 + 2, 1 + 2) - 2) - x) - x) - x, 1, 2 + y) + x, x, 0), 1, y) + x, x, 1) - (x * x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f8 x) (- (- (- (- (w9 x) 2) x) x) x))))
(assert (= g8 1))
