;; sequence(s): A28184
;; terms: 1 31 608 9650 135471 1756461 21553078 254114080 2907590741 32510311691 356977004748 3863433611310 41325142341211 437804739243721 4601385756887618 48040443020269340 498759633052498881 5153605455623560551 53035826592972273688
;; small program: loop((loop(loop(loop((2 * (x + x)) + x, x, loop(x + x, x, 2) - 1), 1, y) + loop((x + x) + x, 2, x), y, 1) + (2 * ((x + x) + x))) + x, x, 1)
;; fast program: loop(((((((((loop(loop((loop((loop(x + x, x, 2) - 1) * loop2(x * y, y, x, 1, 1 + (2 + 2)), 1, y) + x) + (((x + x) + x) * 2), x, 1), 1, y) + x) + x) + x) + x) + x) + x) + x) + x) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f8 x y) (+ (+ (v9 x y) x) (* (+ (+ x x) x) 2)))))
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (+ (+ (+ (+ (+ (+ (+ (+ (+ (v7 x y) x) x) x) x) x) x) x) x) x))))
