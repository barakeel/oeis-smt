;; sequence(s): A28186
;; terms: 1 33 694 11898 181747 2582715 34981048 458355876 5866720453 73835049717 917894484442 11308397377134 138391574753719 1685252212876239 20446364918965276 247382964478976472
;; small program: loop((loop((loop(loop((x + x) + x, y + y, 1) + loop((2 + y) * x, 2, x), y, 1) + (2 * ((x + x) + x))) + x, y, 1) + (2 * (x + x))) + x, x, 1)
;; fast program: loop((loop(loop(loop(loop2(((2 * (x + x)) + (y * y)) + x, (y + y) + y, x, 1, 1 + 2), 1, y) + (2 * (2 * ((x + x) + x))), x, 1), 1, y) + (2 * ((x + x) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f9 x y) (+ (+ (* 2 (+ x x)) (* y y)) x))))
