;; sequence(s): A173142
;; terms: 1 3 22 224 2837 43243 773474 15903604 369769661 9594928683 274906599294 8620383706328 293663289402069 10799919901775579 426469796631518922 17997426089579351788 808344199828497012733
;; small program: loop(loop(loop2((x * y) + x, y, 1 + x, 1, x) - loop2(x * y, y, x, 2, x), 1, y) + x, x, 1)
;; fast program: loop(loop((loop2(x * y, y, x - 1, 1, 1 + x) * (((2 + x) * x) + 1)) - loop2(x * y, y, x, 2, x), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (- (* (w6 x) (+ (* (+ 2 x) x) 1)) (w7 x)))))
(assert (= g5 1))
