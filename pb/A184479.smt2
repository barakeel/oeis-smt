...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (+ (divf (* y y) (+ x y)) y) y))))
(assert (forall ((x Int)) (= (g0 x) (* 2 (+ x x)))))
(assert (= h0 0))
(assert (forall ((x Int) (y Int)) (= (u0 x y) (ite (<= x 0) y (f0 (u0 (- x 1) y) x)))))
(assert (forall ((x Int)) (= (v0 x) (u0 (g0 x) h0))))
(assert (forall ((x Int)) (= (small x) (+ (divf (+ (+ (+ (+ (+ (+ (+ (divf (v0 x) 2) 2) 2) 2) 2) x) 2) x) 2) x))))
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (+ (+ (+ (+ (divf (divf (+ (divf (+ 1 (* 2 (+ 2 (+ x x)))) (+ 1 (+ 2 (* 2 (* 2 (+ 2 2)))))) x) 2) 2) 1) x) 2) x) x) x) 2))))
