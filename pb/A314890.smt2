...
(assert (forall ((x Int)) (= (f1 x) (+ (+ (modf (+ (+ (divf (modf x (+ 1 (* 2 (+ 2 (+ 2 2))))) (+ 1 (* 2 (+ 2 2)))) 1) x) 2) 2) x))))
(assert (= g1 2))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (divf (+ (+ (* (+ (+ (divf (+ 1 (+ 2 (+ 2 x))) (+ 1 (+ 2 (* 2 (+ 2 2))))) (ite (<= x 0) 1 x)) x) 2) 1) x) (+ 1 2)) x) x) x))))
