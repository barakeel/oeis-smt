;; sequence(s): A307663
;; terms: 1 6 41 329 3090 33654 420792 5981688 95782320 1712555280 33909364800 737868052800 17521164259200 451126883894400 12522623670144000 372847351488998400 11853064556660275200 400718191717647820800 14354714544806716416000 543129329390299739136000
;; small program: loop(loop((loop(loop(2 * (x * y), y, 1) + (x * y), y, 1) + (x * y)) + x, y, 1) + (x * y), x, 1)
;; fast program: loop(loop(loop(loop(2 * (x * y), y, 1) + (x * y), y, 1) + ((x * y) + x), y, 1) + (x * y), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (* 2 (* x y)))))
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (+ (v7 x y) (* x y)))))
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (v6 x y) (+ (* x y) x)))))
