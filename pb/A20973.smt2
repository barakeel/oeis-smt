;; sequence(s): A20973
;; terms: 1 28 527 8330 119361 1607088 20736787 259516390 3174562421 38159361548 452427201447 5305418786850 61660643651881 711374339146408 8156868543440507 93047809667655710 1056781696172883741
;; small program: loop((loop(loop(loop((2 + y) * x, 2, x) - x, y, 1) + (2 * ((2 * (x + x)) + x)), y, 1) + (2 * ((x + x) + x))) + x, x, 1)
;; fast program: loop((loop(loop2(((2 * (2 * (x + x))) + x) + y, 2 * ((2 * (y + y)) + x), x, 1, 2 * (2 + 2)), 1, y) + x) + ((((x + x) * 2) + x) * 2), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (g6 x y) (* 2 (+ (* 2 (+ y y)) x)))))
