;; sequence(s): A98310
;; terms: 0 1 11 144 1859 24025 310464 4012009 51845651 669981456 8657913275 111882891121 1445819671296 18683772835729 241443227193179 3120078180675600 40319573121589619 521034372399989449
;; small program: loop(loop2(y, ((2 * (2 * ((y + y) + y))) - x) + y, y, 1, 1) - x, x, 0)
;; fast program: loop(loop2(((2 * (2 * ((x + x) + x))) - y) + x, x, y - 1, 1, 1) - x, x, 0)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (g1 x y) (+ (- (* 2 (* 2 (+ (+ y y) y))) x) y))))
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (+ (- (* 2 (* 2 (+ (+ x x) x))) y) x))))
