;; sequence(s): A59174
;; terms: 1 2 4 8 16 32 64 126 240 438 764 1276 2048 3172 4760 6946 9888 13770 18804 25232
;; small program: ((loop(loop(loop(loop(x + y, y - 2, x), y, x) + y, y - 2, x), x, 1) + x) * 2) - (if x <= 0 then 1 else 2)
;; fast program: ((loop(loop(loop(loop(x + y, y, x), y - 2, x) + y, y - 2, x), x, 1) + x) * 2) - (if x <= 0 then 1 else 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (* (+ (v4 x) x) 2) (ite (<= x 0) 1 2)))))
