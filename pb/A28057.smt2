;; sequence(s): A28057
;; terms: 1 25 408 5558 68999 813603 9315286 104854096 1168677477 12952000061 143075613044 1577575222314 17376397872835 191281848531799 2104964417048082 23159869359140612 254790508985373473 2802889182202991217 30832952040578234800 339169546346404999390
;; small program: loop((loop(loop(loop((x + x) + x, x, loop(x + x, x, 2) - 1), 1, y) + (loop((2 + y) * x, 2, x) - x), y, 1) + (2 * (x + x))) + x, x, 1)
;; fast program: loop((loop(loop((loop((loop(x + x, x, 2) - 1) * loop2(x * y, y, x, 1, 1 + 2), 1, y) + (2 * (x + x))) + x, x, 1), 1, y) + x) + ((((x * 2) * 2) + x) * 2), x, 1)
(set-logic UFNIA)
