;; sequence(s): A28191
;; terms: 1 33 690 11690 175371 2432283 31956940 403804980 4956450741 59506980533 702298022790 8177594495070 94207230682111 1076026739628783 12205736906550240 137681865037361960
;; small program: loop((loop(loop(loop((x + x) + x, y + y, 1) + (loop((2 + y) * x, 2, x) - x), y, 1) + (2 * (2 * (x + x))), y, 1) + (2 * (x + x))) + x, x, 1)
;; fast program: loop((loop(loop((loop2(((2 * (x + x)) + ((y * y) * y)) + x, y + y, y, 1, 2) + (2 * ((2 * (x + x)) + x))) + x, x, 1), 1, y) + x) + (((x + x) * 2) * 2), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f8 x y) (+ (+ (* 2 (+ x x)) (* (* y y) y)) x))))
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (+ (+ (w8 x y) (* 2 (+ (* 2 (+ x x)) x))) x))))
