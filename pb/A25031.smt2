;; sequence(s): A25031
;; terms: 1 32 675 11830 186641 2752932 38765455 527869010 7009691181 91304772832 1171464693035 14850954198990 186462962442121 2322930268904732 28754646814361415 354082225170989770
;; small program: loop(loop(loop(1 + (2 * ((2 * (x + x)) + x)), y, 1) + loop((x + x) + x, 2, x), y, 1) + loop((2 + y) * x, 2, x), x, 1)
;; fast program: loop((loop(loop(loop2(1 + (x * y), y, y, 1, 2 + (2 * (2 + 2))) + ((((x + x) + x) * 2) * 2), x, 1), 1, y) + x) + (((x + x) * 2) * 2), x, 1)
(set-logic UFNIA)
