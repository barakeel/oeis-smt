...
(assert (forall ((x Int)) (= (f0 x) (+ (modf (+ (divf (- (divf (divf x 2) (+ 1 2)) x) (+ (+ 2 2) 2)) x) 2) x))))
(assert (= g0 2))
(assert (forall ((x Int)) (= (h0 x) x)))
(assert (forall ((x Int) (y Int)) (= (u0 x y) (ite (<= x 0) y (f0 (u0 (- x 1) y))))))
(assert (forall ((x Int)) (= (v0 x) (u0 g0 (h0 x)))))
(assert (forall ((x Int)) (= (small x) (modf (+ (v0 x) x) 2))))
(assert (forall ((x Int)) (= (fast x) (modf (modf (+ (+ (* (divf (- (divf (+ (divf (- 2 (divf (+ 1 x) (+ 2 (* 2 (+ 2 2))))) 2) 2) 2) 2) 2) 2) 1) x) (+ (* (+ 2 1) 2) 1)) 2))))
