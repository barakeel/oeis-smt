...
(assert (forall ((x Int)) (= (f0 x) (+ (+ (modf (+ (modf x 2) (modf (divf x (+ 1 2)) (+ 1 2))) 2) 1) x))))
(assert (forall ((x Int)) (= (g0 x) x)))
(assert (= h0 2))
(assert (forall ((x Int) (y Int)) (= (u0 x y) (ite (<= x 0) y (f0 (u0 (- x 1) y))))))
(assert (forall ((x Int)) (= (v0 x) (u0 (g0 x) h0))))
(assert (forall ((x Int)) (= (small x) (+ (+ (+ (+ (- (v0 x) 1) x) x) x) x))))
(assert (forall ((x Int)) (= (fast x) (+ (+ (divf (+ (+ (- (+ (* (+ (divf (+ (* 2 (+ x x)) x) (+ 1 (+ 2 (* 2 (+ 2 2))))) x) 2) 1) (ite (<= x 0) 1 2)) x) x) (+ 1 2)) 1) (* 2 (+ x x))))))
