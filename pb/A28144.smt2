;; sequence(s): A28144
;; terms: 1 33 703 12333 194239 2860821 40291111 549874221 7333588207 96118840389 1242878986999 15899609772189 201637495269055 2538952949123637 31780003009949767 395799688351994637
;; small program: loop((loop(loop(loop(2 * ((x + x) + x), x, loop(x + x, x, 2) - 1), 1, y) + (2 * (x + x)), y, 1) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
;; fast program: loop((loop(loop((loop(loop((x + x) + x, x, loop(x + x, x, 2) - 1), 1, y) + x) + x, x, 1) * loop(x + x, x, 1), 1, y) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (+ (v1 x y) (* 2 (+ (* 2 (+ x x)) x))) x))))
