...
(assert (forall ((x Int)) (= (f3 x) (+ (modf (+ (ite (<= (- (modf (+ 1 (v4 x)) (+ 1 x)) 2) 0) 1 x) x) 2) (modf x 2)))))
(assert (= g3 1))
...
(assert (forall ((x Int)) (= (f0 x) (modf (+ (+ (* (v1 x) 2) 1) x) (+ 1 x)))))
...
(assert (forall ((x Int)) (= (f7 x) (modf (+ (+ (ite (<= (- (modf (+ 1 (v8 x)) (+ 1 (+ x x))) 2) 0) 1 2) x) x) (ite (<= x 0) 1 2)))))
(assert (= g7 1))
(assert (forall ((x Int)) (= (h7 x) (divf x 2))))
(assert (forall ((x Int) (y Int)) (= (u7 x y) (ite (<= x 0) y (f7 (u7 (- x 1) y))))))
(assert (forall ((x Int)) (= (v7 x) (u7 g7 (h7 x)))))
(assert (forall ((x Int)) (= (f6 x) (+ (+ (modf (+ (+ (- (modf (+ (v7 x) x) 2) (modf x 2)) 1) x) 2) (modf x 2)) x))))
(assert (= g6 2))
