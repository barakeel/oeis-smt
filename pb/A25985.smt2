;; sequence(s): A25985
;; terms: 1 29 563 9241 138723 1972833 27071851 362271257 4758885395 61635081937 789436108539 10021208067273 126282252770467 1581692483050241 19709670281308427 244539010854912889
;; small program: loop((loop(loop2((loop((2 + y) * x, 2, x) - x) + (y * y), y + y, y, 1, 2) + loop((2 + y) * x, 2, x), y, 1) + x) + x, x, 1)
;; fast program: loop((loop((loop((2 * ((x + x) + x)) - 1, x, 2) - loop(x + x, x, 1)) * loop(x + x, x, 1), 1, y) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f7 x) (- (* 2 (+ (+ x x) x)) 1))))
...
(assert (forall ((x Int)) (= (f6 x) (* (- (v7 x) (v8 x)) (v9 x)))))
(assert (= g6 1))
