;; sequence(s): A28128
;; terms: 1 31 623 10355 155151 2182251 29453143 386424115 4968644351 62947998971 788663289063 9797059846275 120896875524751 1484089915071691 18142382512443383 221041773758074835
;; small program: loop(loop(loop(loop((2 * (x + x)) + x, x, loop(x + x, x, 2) - 1), 1, y) + (2 * (x + x)), y, 1) + loop((2 + y) * x, 2, x), x, 1)
;; fast program: loop(loop(loop(loop((loop(x + x, x, 2) - 1) * loop2(x * y, y, x, 1, 1 + (2 + 2)), 1, y) + (2 * (x + x)), x, 1), 1, y) + (2 * (2 * ((x + x) + x))), x, 1)
(set-logic UFNIA)
