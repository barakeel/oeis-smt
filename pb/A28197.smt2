;; sequence(s): A28197
;; terms: 1 36 823 15270 251161 3822096 55123003 764718570 10305415021 135822156756 1759270703983 22475843025870 283991223372481 3556423423325016 44213833662081763 546396746667037170
;; small program: loop(loop(loop(loop((2 * (x + x)) + x, x, loop(x + x, x, 2) - 1), 1, y) + loop((x + x) + x, 2, x), y, 1) + loop((2 + y) * x, 2, x), x, 1)
;; fast program: loop(loop((loop(loop((2 * (x + x)) + x, x, loop(x + x, x, 2) - 1), 1, y) + (2 * (2 * (x + x)))) + x, y, 1) + ((((x + x) + x) * 2) * 2), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f10 x) (+ (* 2 (+ x x)) x))))
...
(assert (forall ((x Int)) (= (h10 x) (- (v11 x) 1))))
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (+ (v8 x y) (* (* (+ (+ x x) x) 2) 2)))))
