;; sequence(s): A26149
;; terms: 1 28 517 7980 111741 1473948 18687397 230431180 2784091981 33121756668 389323047477 4532472142380 52356786665821 600922618820188 6860138634498757 77961474116861580 882575960972337261
;; small program: loop((loop(loop(loop((2 * (x + x)) + x, x, loop(x + x, x, 2) - 1), 1, y) + (loop((2 + y) * x, 2, x) - x), y, 1) + x) + x, x, 1)
;; fast program: loop((loop(loop((loop((loop(x + x, x, 2) - 1) * loop2(x * y, y, x, 1, 1 + (2 + 2)), 1, y) + x) + x, x, 1), 1, y) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
(set-logic UFNIA)
