;; sequence(s): A125048
;; terms: 0 24 48576 73741824 99511627776 125899906842624 152921504606846976 180591620717411303424 208925819614629174706176 237940039285380274899124224 267650600228229401496703205376
;; small program: loop(x + x, 2 * ((2 * (x + x)) + x), 1) - loop(2 * ((2 * (x + x)) + x), (x + x) + x, 1)
;; fast program: (loop(x * x, 2, loop(x + x, x, 1)) * loop((x * x) * x, 1, loop(x * x, 1, loop(x + x, x, 1)))) - loop((x * x) * x, 1, loop2(x * y, y, x, 1, 2 + (2 * (2 + 2))))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (* (v2 x) (v4 x)) (v7 x)))))
