;; sequence(s): A36099
;; terms: 1 2097153 10462450355 4408506864307 481235204714229 22413787798580981 580482814723661863 9781917900938059815 118642361168367135017 1109418989131512359209 8400249944258160101211 53405369853627861567323
;; small program: loop2(loop(loop2((((x * x) * x) * x) * y, y, 1 + 2, 1, x), 1, y) + x, 1 + y, 2, 0, x)
;; fast program: loop2(loop(loop2((((y * y) * y) * x) * x, y, 1 + 2, 1, x), 1, y) + x, 1 + y, 2, 0, x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (* (* (* x x) x) x) y))))
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (* (* (* (* y y) y) x) x))))
