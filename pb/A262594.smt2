;; sequence(s): A262594
;; terms: 1 4 14 52 203 808 3232 12936 51765 207100 828466 3313964 13255999 53024192 212097028 848388448 3393554217 13574217396 54296870230 217187481700
;; small program: loop(loop(loop(loop(1 + (2 * (x + x)), y - 1, 1) + x, y, 1) + x, y, x), x, 1) + x
;; fast program: loop((((loop(loop(loop(x + y, y - 2, x) - y, x, 1), 1, y) + x) + x) + x) + (if (x - 1) <= 0 then 1 else x), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (+ (+ (+ (+ (v5 x y) x) x) x) (ite (<= (- x 1) 0) 1 x)))))
