;; sequence(s): A28204
;; terms: 1 32 645 10480 150101 1979712 24649045 294242960 3402478101 38387226592 424748805845 4627041422640 49770222820501 529800364460672 5591164247433045 58580499720429520 610040416273729301
;; small program: loop((loop(loop(loop((x + x) + x, y + y, 1) + (2 * ((x + x) + x)), y, 1) + (2 * ((2 * (x + x)) + x)), y, 1) + (2 * ((x + x) + x))) + x, x, 1)
;; fast program: loop((loop(loop(loop(loop2(((x + x) + x) + y, y + y, x, 1, 2) * loop2(x * y, y, x, 1, 1 + 2), 1, y) + (2 * ((2 * (x + x)) + x)), x, 1), 1, y) + (2 * ((x + x) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f8 x y) (+ (+ (+ x x) x) y))))
