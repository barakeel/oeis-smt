;; sequence(s): A138586
;; terms: 1 129 280065 4586751489 358322666751489 100306488365546751489 82606511560391889386751489 173238283180457843219993066751489 828593116199250458889895450218986751489
;; small program: loop(loop2(loop(x + (y * x), y, x) * x, y, 1 + 2, 1, y) + x, x, 1)
;; fast program: loop2(1 + (((((((y * y) * y) * y) * y) * y) * y) * x), y - 1, x, 1, 1 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (+ 1 (* (* (* (* (* (* (* y y) y) y) y) y) y) x)))))
