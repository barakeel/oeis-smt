;; sequence(s): A13805
;; terms: 4096 268435456 17592186044416 1152921504606846976 75557863725914323419136 4951760157141521099596496896 324518553658426726783156020576256 21267647932558653966460912964485513216 1393796574908163946345982392040522594123776
;; small program: loop(x + x, loop(2 * ((x + x) + y), 2, x), 1)
;; fast program: loop((((x * x) * x) * x) * x, 1, loop(x * x, 1, loop(x + x, x, 1))) * loop((x * x) * x, 1, loop(x * x, 1, 2 * loop(x + x, x, 2)))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (h6 x) (* 2 (v7 x)))))
