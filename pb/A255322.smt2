;; sequence(s): A255322
;; terms: 1 1 24 8709120 182219087869378560000 2826438545846116156142906806150103040000000000 1051416277636507481568264360276689674557030810000137484550133942059008000000000000000000
;; small program: loop(loop(x * y, y * y, x), x, 1)
;; fast program: loop(loop(loop(loop((loop2(((y * y) + y) * x, 2 + y, x, 1, 1 + (x * x)) * (1 + x)) * (1 + x), 1, y) * x, x, 1), 1, y) * x, x - 1, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f5 x) (* (* (w6 x) (+ 1 x)) (+ 1 x)))))
(assert (= g5 1))
