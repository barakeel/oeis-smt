;; sequence(s): A16092
;; terms: 1 40 1005 20300 360521 5881680 90375685 1328528500 18879767841 261281940920 3540146411165 47147861315100 619093593987961 8034113714073760 103234967777291445 1315472793832108100
;; small program: loop((loop(loop(loop((x + x) + x, y + y, 1) + (2 * (2 * (x + x))), y, 1) + loop((2 + y) * x, 2, x), y, 1) - x) + loop((2 + y) * x, 2, x), x, 1)
;; fast program: loop((loop(loop((loop(loop2(((x + x) + x) + y, y + y, x, 1, 2) * loop(x * x, 1, loop(x + x, x, 1)), 1, y) + x) + (((x * 2) * 2) * 2), x, 1), 1, y) + x) + ((((x * 2) * 2) + x) * 2), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f10 x y) (+ (+ (+ x x) x) y))))
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (+ (+ (v7 x y) x) (* (+ (* (* x 2) 2) x) 2)))))
