;; sequence(s): A22448
;; terms: 1 24 391 5466 71017 888660 10902187 132353502 1597893853 19235078616 231195264703 2776650139458 33333948627409 400093791131292 4801651568584339 57623013420037734 691495519357470085 8298063335866061088 99577467420222835495 1194933877223512550730
;; small program: loop((loop(loop(1 + (2 * ((x + x) + x)), y, 1) + loop((2 + y) * x, 2, x), y, 1) + (2 * (x + x))) + x, x, 1)
;; fast program: loop(loop(loop2((2 * ((x + x) + x)) + y, 1 + ((2 * (y + y)) + y), x, 1, 2 + (2 + 2)), 1, y) + (2 * (2 * ((x + x) + x))), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (g6 x y) (+ 1 (+ (* 2 (+ y y)) y)))))
