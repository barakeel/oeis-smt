;; sequence(s): A217471
;; terms: 0 1 244 33012 4117113 507401488 62424765712 7678070811369 944346243245076 116147016764564500 14285140634333292625 1756956185432949082176 216091326285380812359744 26577476188001703626949937
;; small program: loop(loop(loop(loop(x * x, 2, x) * x, 1, loop2(x + y, x, x + x, 0, 1)), 1, y) + x, x, 0)
;; fast program: loop(loop(loop((((x * x) * x) * x) * x, 1, loop2(x + y, x, (x - 2) + x, 1, 1)), 1, y) + x, x, 0)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (h8 x) (+ (- x 2) x))))
(assert (= i8 1))
(assert (= j8 1))
