;; sequence(s): A296627
;; terms: 0 2 24883200 6658606584104736522240000000 69113789582492712943486800506462734562847413501952000000000000000
;; small program: loop(loop(loop(x * y, y - 2, x), y + y, 1), x + x, 0)
;; fast program: loop(loop(loop(x * y, y - 1, y) * x, x - 2, 1), if x <= 0 then 0 else 1, ((x + x) + x) + x)
(set-logic UFNIA)
