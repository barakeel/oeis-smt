;; sequence(s): A21304
;; terms: 1 24 403 5886 80113 1046748 13329631 166873722 2064748525 25337358072 309091680859 3754490630358 45462401712937 549225447909396 6623795021132887 79783347069641394 960080426180248549
;; small program: loop((loop(loop(1 + loop((x + x) + x, 2, x), y, 1) + loop((2 + y) * x, 2, x), y, 1) + x) + x, x, 1)
;; fast program: loop((loop2((2 * ((2 * ((x + x) + x)) + y)) - 1, y + y, y, 1, 2) + (2 * (2 * (x + x)))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (- (* 2 (+ (* 2 (+ (+ x x) x)) y)) 1))))
