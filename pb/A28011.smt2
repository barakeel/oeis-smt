;; sequence(s): A28011
;; terms: 1 30 589 9606 141469 1956918 25981309 335413254 4245767581 52992982614 654705201085 8028335357094 97906258582237 1189106856272118 14398317567128317 173947775018406726 2097931504239454237 25270527569760142230 304106756286138899005
;; small program: loop((loop((loop(loop((x + x) + x, y + y, 1) + loop((2 + y) * x, 2, x), y, 1) + (2 * ((x + x) + x))) + x, y, 1) + x) + x, x, 1)
;; fast program: loop((loop(loop((loop(loop(x + x, x, loop2(1 + (x * y), y, x, 1, 2 + (2 + 2))), 1, y) + (2 * (2 * (x + x)))) + x, x, 1), 1, y) + (((x + x) + x) * 2)) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f10 x y) (+ 1 (* x y)))))
