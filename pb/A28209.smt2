;; sequence(s): A28209
;; terms: 1 36 823 15282 251881 3847032 55777051 779149134 10588135261 140901434388 1844713666159 23841089487306 304926682591441 3866889746187504 48692419436767747 609526612369569798
;; small program: loop((loop(loop(loop(2 * ((x + x) + x), x, loop(x + x, x, 2) - 1), 1, y) + (loop((2 + y) * x, 2, x) - x), y, 1) + (2 * ((x + x) + x))) + x, x, 1)
;; fast program: loop((loop(loop((loop((loop(x + x, x, 2) - 1) * loop2(x * y, y, x, 1, 2 + (2 + 2)), 1, y) + (2 * ((x + x) + x))) + x, x, 1), 1, y) + x) + ((((x * 2) * 2) + x) * 2), x, 1)
(set-logic UFNIA)
