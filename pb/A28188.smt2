;; sequence(s): A28188
;; terms: 1 34 737 13022 204885 2995842 41695033 560476774 7346285309 94497418730 1198353224289 15030879260046 186924790959973 2308975949692498 28368854500004105 347054225208330038
;; small program: loop((loop(loop(loop((2 * (x + x)) + x, x, loop(x + x, x, 2) - 1), 1, y) + loop((2 + y) * x, 2, x), y, 1) + (2 * ((x + x) + x))) + x, x, 1)
;; fast program: loop(loop((loop(loop((2 * (x + x)) + x, x, loop(x + x, x, 2) - 1), 1, y) + (2 * ((x + x) + x))) + x, y, 1) + ((((x + x) + x) * 2) * 2), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f9 x) (+ (* 2 (+ x x)) x))))
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (+ (v7 x y) (* (* (+ (+ x x) x) 2) 2)))))
