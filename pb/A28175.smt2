;; sequence(s): A28175
;; terms: 1 31 612 9842 140903 1874733 23747074 290631904 3469802325 40674188555 470287309856 5380934171886 61072079752867 688801372689097 7730331225654558 86417957138887388 963072515157938129
;; small program: loop((loop(loop(loop((x + x) + x, y + y, 1) + (loop((2 + y) * x, 2, x) - x), y, 1) + (2 * ((x + x) + x)), y, 1) + (2 * (x + x))) + x, x, 1)
;; fast program: loop((loop(loop((loop(loop2(((x + x) + x) + y, y + y, x, 1, 2) * loop2(x * y, y, x, 1, 1 + 2), 1, y) + (2 * (x + x))) + x, x, 1), 1, y) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
(set-logic UFNIA)
