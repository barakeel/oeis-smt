;; sequence(s): A181179
;; terms: 1 2 4 48 24827831185072145341724578094746750077042972709355520000000000
;; small program: loop(loop(x * y, x, 2), x, 1)
;; fast program: loop(loop(x * y, x, 1 + x) * loop2(x * y, 1 + y, x - 1, 1, 2 + x), x - 1, 1) * (if x <= 0 then 1 else 2)
(set-logic UFNIA)
