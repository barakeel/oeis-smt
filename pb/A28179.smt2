;; sequence(s): A28179
;; terms: 1 34 741 13226 211037 3139050 44543077 611425162 8191217133 107740573226 1397106491573 17913626282058 227607187590589 2870438906000362 35976385743457029 448561605029480714 5568050161261720205 68855119590387513258 848685719578619420245
;; small program: loop((loop(loop(loop(2 * ((x + x) + x), x, loop(x + x, x, 2) - 1), 1, y) + (loop((2 + y) * x, 2, x) - x), y, 1) + (2 * (x + x))) + x, x, 1)
;; fast program: loop((loop(loop((loop((loop(x + x, x, 2) - 1) * loop2(x * y, y, x, 1, 2 + (2 + 2)), 1, y) + (2 * (x + x))) + x, x, 1), 1, y) + x) + ((((x * 2) * 2) + x) * 2), x, 1)
(set-logic UFNIA)
