;; sequence(s): A192389
;; terms: 0 1 3 9 22 48 96 181 327 573 982 1656 2760 4561 7491 12249 19966 32472 52728 85525
;; small program: loop(loop(loop2(2 + (x + y), x, y - 2, 1, 1) + x, y, x), x, 0)
;; fast program: ((((((loop2(x + y, x, x, 1 + 2, 2) - 2) - x) * (1 + 2)) - 1) - 2) - (x * x)) - x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (- (- (- (- (* (- (- (w3 x) 2) x) (+ 1 2)) 1) 2) (* x x)) x))))
