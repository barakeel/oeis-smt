;; sequence(s): A28143
;; terms: 1 32 660 11200 170576 2429952 33121600 437888000 5663851776 72087887872 906450621440 11292447129600 139666259070976 1717603091873792 21027551488327680 256495170125824000
;; small program: loop(loop(loop(loop(2 * ((x + x) + x), x, loop(x + x, x, 2) - 1), 1, y) + (2 * ((2 * (x + x)) + x)), y, 1) + (2 * (x + x)), x, 1)
;; fast program: loop((loop((loop(loop((x + x) + x, x, loop(x + x, x, 2) - 1), 1, y) + x) + x, y, 1) + (2 * (x + x))) + x, x, 1) * loop(x + x, x, 1)
(set-logic UFNIA)
