;; sequence(s): A114641
;; terms: 1 2 4 16 256 16777216 1942668892225729070919461906823518906642406839052139521251812409738904285205208498176
;; small program: loop2(loop(x + x, y, x), x, x, 1, 1)
;; fast program: loop(loop(loop2(loop2(x * y, y, y - 2, 1, x) * (x * x), x, x - 2, if x <= 0 then 1 else 2, 1), 1, y) * x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (h5 x y) (- y 2))))
(assert (= i5 1))
