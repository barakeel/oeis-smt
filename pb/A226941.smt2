;; sequence(s): A226941
;; terms: 1 35 798 15178 262739 4310073 68451856 1065454016 16372593237 249520885471 3782278181474 57129692163414 860905800344695 12953222527379429 194694881199600852 2924389779305546572 43905519073297744313 658979550560400579147 9888661146758667705190
;; small program: loop(loop(loop(((loop(loop(1 + ((loop(x * x, 2, 2) * x) - x), y, 1) + (2 * ((2 * (x + x)) + x)), y, 1) + x) + x) + x, x, 1), 1, y) + (((x + x) + x) * 2), x, 1)
;; fast program: loop((((((loop(loop(loop(loop2((2 * ((x + x) + x)) + y, 1 + ((y + y) + y), x, 1, 2 + 2), 1, y) + (2 * ((2 * (x + x)) + x)), x, 1), 1, y) + (2 * ((2 * (x + x)) + x))) + x) + x) + x) + x) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (g10 x y) (+ 1 (+ (+ y y) y)))))
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (+ (+ (+ (+ (+ (+ (v7 x y) (* 2 (+ (* 2 (+ x x)) x))) x) x) x) x) x))))
