;; sequence(s): A28177
;; terms: 1 32 653 10864 160965 2216256 29037037 367278608 4526901269 54714980320 651369416061 7662042543792 89264097643813 1031793631351424 11849055184966925 135335527957665616
;; small program: loop(loop(loop(loop((2 * (x + x)) + x, x, loop(x + x, x, 2) - 1), 1, y) + (2 * ((x + x) + x)), y, 1) + (loop((2 + y) * x, 2, x) - x), x, 1)
;; fast program: loop((loop(loop(loop((2 * (x + x)) + x, x, loop(x + x, x, 2) - 1), 1, y) + (2 * ((x + x) + x)), y, 1) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
(set-logic UFNIA)
