;; sequence(s): A28215
;; terms: 1 37 867 16457 276563 4296369 63218779 894460009 12289771395 165102658721 2179224895211 28361892985881 364934200067347 4651876568586193 58839203762144763 739393306409353673
;; small program: loop(loop(loop(loop(2 * ((x + x) + x), x, loop(x + x, x, 2) - 1), 1, y) + (2 * (2 * (x + x))), y, 1) + (loop((2 + y) * x, 2, x) - x), x, 1)
;; fast program: loop((loop(loop(loop(loop((x + x) + x, x, loop(x + x, x, 2) - 1), 1, y) + (2 * (x + x)), x, 1) * loop(x + x, x, 1), 1, y) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
(set-logic UFNIA)
