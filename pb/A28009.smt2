;; sequence(s): A28009
;; terms: 1 28 509 7628 102477 1284060 15338893 177026476 1991073293 21954504572 238334455437 2555217498444 27118409281549 285415846100764 2983193752870541 30999901741572332 320557221434296845 3300921733229659836 33869642781313790605
;; small program: loop((loop((loop(loop((x + x) + x, y + y, 1) + (2 * ((2 * (x + x)) + x)), y, 1) + (2 * ((x + x) + x))) + x, y, 1) + x) + x, x, 1)
;; fast program: loop((loop(loop((loop(loop2(1 + (x * y), y, x, 1, 1 + (2 + 2)) * loop(x + x, x, 1), 1, y) + x) + (((x + x) * 2) * 2), x, 1), 1, y) + x) + (((x + x) + x) * 2), x, 1)
(set-logic UFNIA)
