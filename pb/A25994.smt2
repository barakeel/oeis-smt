;; sequence(s): A25994
;; terms: 1 24 377 4932 58485 654192 7053313 74243004 769050029 7880095080 80144389689 810929347956 8176098535333 82230655243488 825604414181105 8279180036642988 82954005943360797 830676770037792216
;; small program: loop((loop((loop(loop((2 * (x + x)) + x, x, loop(x + x, x, 2) - 1), 1, y) + (2 * ((x + x) + x))) + x, y, 1) + x) + x, x, 1)
;; fast program: loop((loop(loop((loop((loop(x + x, x, 2) - 1) * loop2(x * y, y, x, 1, 1 + (2 + 2)), 1, y) + x) + x, x, 1), 1, y) + x) + (((x + x) + x) * 2), x, 1)
(set-logic UFNIA)
