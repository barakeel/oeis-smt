;; sequence(s): A204247
;; terms: 1 0 ~1 ~8 ~204 ~24768 ~17867520 ~90077184000 ~3632037470208000 ~1317998813773824000000 ~4782755930355924782284800000 ~190912318579437962454242426880000000
;; small program: loop(loop(x * y, y - 1, x), x, 1) * loop((x * y) - 1, x, 1)
;; fast program: (loop(loop(x * y, y - 1, y) * x, x - 2, 1) * loop(x * y, x - 1, 1)) * loop((x * y) - 1, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (- (* x y) 1))))
