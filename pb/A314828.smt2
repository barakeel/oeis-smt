...
(assert (forall ((x Int)) (= (f1 x) (+ (modf (- (- (divf x (* 2 (+ 2 2))) (modf x 2)) 2) (+ 1 (+ 2 2))) 1))))
(assert (= g1 2))
...
(assert (forall ((x Int)) (= (f3 x) (+ (+ (modf (divf (* x x) (* 2 (+ 2 (+ 2 2)))) 2) 1) x))))
...
(assert (forall ((x Int)) (= (fast x) (+ (+ (+ (divf (+ (+ (- (+ (+ (+ (+ (+ (v2 x) 1) x) x) x) x) (ite (<= x 0) 1 2)) x) x) (+ 1 2)) 1) x) x))))
