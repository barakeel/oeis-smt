;; sequence(s): A28125
;; terms: 1 29 542 8338 115203 1490127 18459184 221986196 2613955685 30311197345 347486967906 3949158184374 44583107459047 500701324008083 5600328285552308 62437282927169272 694313904033290889
;; small program: loop((loop(loop(loop((x + x) + x, y + y, 1) + (loop((2 + y) * x, 2, x) - x), y, 1) + (2 * (x + x)), y, 1) + (2 * (x + x))) + x, x, 1)
;; fast program: loop((loop(loop(loop2(((2 * (x + x)) + (y * y)) + x, y + y, y, 1, 2) + ((1 + 2) * ((x + x) + x)), x, 1), 1, y) + ((((x + x) * 2) + x) * 2)) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f8 x y) (+ (+ (* 2 (+ x x)) (* y y)) x))))
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (+ (w8 x y) (* (+ 1 2) (+ (+ x x) x))))))
