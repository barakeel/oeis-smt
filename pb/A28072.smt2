;; sequence(s): A28072
;; terms: 1 30 589 9630 142621 1989510 26692669 348689550 4469016541 56480471190 706312706749 8761344810270 107992670570461 1324480424684070 16179464126208829 197010636086927790
;; small program: loop(((loop(loop(loop((2 * (x + x)) + x, x, loop(x + x, x, 2) - 1), 1, y) + loop((2 + y) * x, 2, x), y, 1) + x) + x) + x, x, 1)
;; fast program: loop(loop(((loop(loop((2 * (x + x)) + x, x, loop(x + x, x, 2) - 1), 1, y) + x) + x) + x, y, 1) + (2 * (2 * ((x + x) + x))), x, 1)
(set-logic UFNIA)
