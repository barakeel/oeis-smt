;; sequence(s): A6106
;; terms: 1 85 5797 376805 24208613 1550842085 99277752549 6354157930725 406672215935205 26027119554103525 1665737215212030181 106607206793565997285 6822861635108183247077 436663151052043168024805 27946441769812674154891493
;; small program: loop(loop(loop(loop(1 + (loop(x * x, 2, 2) * x), y, 1) + (2 * (x + x)), x, 1) * loop(x + x, x + x, 1), 1, y) + x, x, 1)
;; fast program: loop(loop(loop(loop2(1 + ((y * y) * x), y, y, 1, 2 + 2) + ((x * 2) * 2), x, 1) * loop2(x * y, y, x, 1, 2 + 2), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f1 x) (* (v2 x) (v5 x)))))
(assert (= g1 1))
...
(assert (forall ((x Int) (y Int)) (= (f9 x y) (+ 1 (* (* y y) x)))))
...
(assert (forall ((x Int) (y Int)) (= (f8 x y) (+ (w9 x y) (* (* x 2) 2)))))
...
(assert (forall ((x Int)) (= (f7 x) (* (v8 x) (w10 x)))))
(assert (= g7 1))
