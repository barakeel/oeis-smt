;; sequence(s): A254682
;; terms: 1 37 418 2754 13080 49632 159654 452166 1157013 2724865 5988268 12410476 24456744 46132152 83740980 146935284 250134753 414416277 669990046 1059399550
;; small program: loop(loop(loop(loop(loop((loop(x * x, 2, y) * y) + x, y, x), y, x), y, x), 1 + y, x), x, 1)
;; fast program: loop(loop(loop(loop(loop(loop(((((y * y) * y) * y) * y) + x, y, x), y, x), 1 + y, x), x, 1), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f11 x y) (+ (* (* (* (* y y) y) y) y) x))))
