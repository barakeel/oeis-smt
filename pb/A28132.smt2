;; sequence(s): A28132
;; terms: 1 27 465 6535 81921 956487 10657585 115031895 1214419041 12622599847 129754165905 1323318833655 13420125351361 135548860481607 1365156409213425 13720647130385815 137698507311406881
;; small program: loop((loop(loop2((2 * ((x + x) + x)) + (y * y), y + y, y, 1, 2) + (2 * ((2 * (x + x)) + x)), y, 1) + (2 * ((x + x) + x))) + x, x, 1)
;; fast program: loop((loop(loop((loop2(((x + x) + x) + y, y + y, y, 1, 2) + (2 * (x + x))) + x, x, 1) * loop(x + x, x, 1), 1, y) + (2 * ((x + x) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (* 2 (+ (+ x x) x)) (* y y)))))
