;; sequence(s): A28004
;; terms: 1 31 633 10787 166481 2416827 33681481 456072739 6046236801 78876961163 1016157547769 12960408919731 163959874019761 2060322716086939 25744836645629097 320169349603453763
;; small program: loop(loop((loop(loop(2 * ((x + x) + x), x, loop(x + x, x, 2) - 1), 1, y) + x) + x, y, 1) + (loop((2 + y) * x, 2, x) - x), x, 1)
;; fast program: loop((loop(loop2((2 * ((x + x) + x)) + y, 1 + ((y + y) + y), x, 1, 2 + 2) * loop(x + x, x, 1), 1, y) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f8 x y) (+ (* 2 (+ (+ x x) x)) y))))
