;; sequence(s): A324268
;; terms: 31 4992581 804060162631 129494693251885181 20855249842909360285231 3358758842450395383296737781 540931470335478626875322916367831 87117554228999168336897631003955550381 14030369226134545059825700370818045344410431 2259604994238194616429988870420617020762644322981
;; small program: loop(loop((2 + y) * x, 2, x) - x, (2 * (x + x)) + x, 1) * ((2 * loop(x * x, 2, 2)) - 1)
;; fast program: (loop((x * x) * x, 1, loop2(x * y, y, x, 1, 1 + (2 + (2 * (2 + 2))))) * loop(x * x, 1, loop2(x * y, y, x, 1, 1 + (2 + (2 * (2 + 2)))))) * loop(1 + (2 * ((2 * (x + x)) + x)), 1, 1 + 2)
(set-logic UFNIA)
...
(assert (= v2 (u2 g2 h2)))
(assert (forall ((x Int)) (= (small x) (* (v0 x) (- (* 2 v2) 1)))))
...
(assert (forall ((x Int)) (= (f7 x) (+ 1 (* 2 (+ (* 2 (+ x x)) x))))))
(assert (= g7 1))
(assert (= h7 (+ 1 2)))
(assert (forall ((x Int) (y Int)) (= (u7 x y) (ite (<= x 0) y (f7 (u7 (- x 1) y))))))
(assert (= v7 (u7 g7 h7)))
(assert (forall ((x Int)) (= (fast x) (* (* (v3 x) (v5 x)) v7))))
