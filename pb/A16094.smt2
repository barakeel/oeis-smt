;; sequence(s): A16094
;; terms: 1 42 1105 23310 431221 7309722 116419465 1769717670 25948716541 369730963602 5147200519825 70298695224030 944897655707461 12530341519244682 164265473257148185 2132247784185258390
;; small program: loop((loop(loop(loop((x + x) + x, y + y, 1) + (2 * ((2 * (x + x)) + x)), y, 1) + loop((2 + y) * x, 2, x), y, 1) - x) + loop((2 + y) * x, 2, x), x, 1)
;; fast program: loop((loop(loop(loop(loop2((((y * y) + x) + x) + x, y + y, x, 1, 2) * loop2(x * y, y, x, 1, 1 + 2), 1, y) + (2 * ((2 * (x + x)) + x)), x, 1), 1, y) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (- (v1 x y) x) (v5 x)))))
(assert (forall ((x Int)) (= (g0 x) x)))
(assert (= h0 1))
(assert (forall ((x Int) (y Int)) (= (u0 x y) (ite (<= x 0) y (f0 (u0 (- x 1) y) x)))))
(assert (forall ((x Int)) (= (v0 x) (u0 (g0 x) h0))))
(assert (forall ((x Int)) (= (small x) (v0 x))))
(assert (forall ((x Int) (y Int)) (= (f10 x y) (+ (+ (+ (* y y) x) x) x))))
(assert (forall ((x Int) (y Int)) (= (g10 x y) (+ y y))))
...
(assert (forall ((x Int)) (= (f9 x) (* (w10 x) (w11 x)))))
(assert (= g9 1))
