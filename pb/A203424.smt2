;; sequence(s): A203424
;; terms: 1 ~4 ~144 73728 737280000 ~183458856960000 ~1381360067999170560000 370806019753548356895375360000 4086267719027580129096614223921807360000 ~2092169072142121026097466482647965368320000000000000
;; small program: loop(loop(loop2(0 - ((x + x) * y), y, x, 1, 1 + x), 1, y) * x, x, 1)
;; fast program: loop(loop(loop2(0 - (x * y), y, x, 1, 1 + x), 1, y) * x, x, 1) * loop2(x * y, y + y, x, 1, 2)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (- 0 (* (+ x x) y)))))
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (- 0 (* x y)))))
