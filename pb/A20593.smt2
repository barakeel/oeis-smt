;; sequence(s): A20593
;; terms: 1 25 423 6053 79079 977613 11662351 135834661 1556251287 17625401981 197992990559 2211194243349 24591484236775 272666167778029 3016684939110447 33322861263616517 367668910476901943 4053314434522328157 44658211693913532415
;; small program: loop((loop2((2 * ((x + x) + x)) + ((y * y) * y), y + y, y, 1, 2) - x) + loop((2 + y) * x, 2, x), x, 1)
;; fast program: loop((loop(loop2((((y * y) + x) + x) + x, y + y, x, 1, 2) * loop(x + x, x - 1, 2), 1, y) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (+ (+ (* y y) x) x) x))))
