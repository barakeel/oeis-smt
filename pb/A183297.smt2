...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ 1 (divf (+ x y) (+ (* 2 (+ x x)) x))))))
...
(assert (= h4 (+ 1 2)))
(assert (= i4 (+ 2 2)))
(assert (forall ((x Int)) (= (j4 x) (divf x (+ 1 (+ 2 2))))))
...
(assert (forall ((x Int)) (= (f2 x) (+ (+ (modf (- (- (v3 x) 1) x) (+ 1 x)) 1) x))))
(assert (= g2 2))
(assert (forall ((x Int)) (= (h2 x) x)))
(assert (forall ((x Int) (y Int)) (= (u2 x y) (ite (<= x 0) y (f2 (u2 (- x 1) y))))))
(assert (forall ((x Int)) (= (v2 x) (u2 g2 (h2 x)))))
(assert (forall ((x Int)) (= (f1 x) (+ (divf (+ (divf (- (v2 x) x) 2) 1) 2) x))))
(assert (= g1 2))
