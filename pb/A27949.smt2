;; sequence(s): A27949
;; terms: 1 4 25 97 309 894 2462 6610 17519 46135 121115 317484 831660 2177872 5702389 14929789 39087537 102333450 267913514 701407870
;; small program: loop(loop((loop2(x + y, x, y, 2, 1) - 1) + x, y, 1) + x, (x - 1) + x, 1)
;; fast program: loop(loop(((((loop2(x + y, x, x, 1 + (2 + 2), 1 + 2) - 1) - 2) - 2) - x) - x, 1, y + y) + x, x, 1) - (if x <= 0 then 0 else 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f1 x y) (+ (- (w2 x y) 1) x))))
...
(assert (forall ((x Int)) (= (f4 x) (- (- (- (- (- (w5 x) 1) 2) 2) x) x))))
(assert (= g4 1))
