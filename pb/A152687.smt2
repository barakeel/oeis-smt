;; sequence(s): A152687
;; terms: 1 1 2 24 8640 746496000 201231433728000000 3554168771933456302080000000000 139840535301953855934724122328694784000000000000000 674129921807822677705190163721626103970522196466131271680000000000000000000000
;; small program: loop(loop(loop(loop2(x + y, x, y, x, 0), y, x), y, x), x, 1)
;; fast program: loop(loop(loop(loop(loop2(x + y, x, y, 2, 1) * x, y - 1, 1) * x, x, 1), 1, y) * x, x - 1, 1) * loop2(x * y, y + y, x - 1, 1, 2)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (* (w8 x y) x))))
