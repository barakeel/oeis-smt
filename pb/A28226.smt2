;; sequence(s): A28226
;; terms: 1 38 909 17518 297437 4648350 68550973 968960366 13264544733 177146894782 2320188183677 29920806791694 381057655784989 4803870896957534 60058812824159421 745739268125744302
;; small program: loop((loop(loop(loop((x + x) + x, y + y, 1) + (2 * ((2 * (x + x)) + x)), y, 1) + loop((2 + y) * x, 2, x), y, 1) + (2 * ((x + x) + x))) + x, x, 1)
;; fast program: loop((loop(loop(loop(loop2((((y * y) + x) + x) + x, y + y, x, 1, 2) * loop2(x * y, y, x, 1, 1 + 2), 1, y) + (2 * ((2 * (x + x)) + x)), x, 1), 1, y) + (2 * ((x + x) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f9 x y) (+ (+ (+ (* y y) x) x) x))))
