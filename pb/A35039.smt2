;; sequence(s): A35039
;; terms: 0 0 0 0 0 0 0 1 9 46 176 562 1586 4096 9908 22819 50643 109294 230964 480492
;; small program: loop(loop(loop(loop(loop(loop(x + y, (y - 2) - 2, x), y, x), y, x), y - 2, x), y, x) + x, x, 0)
;; fast program: loop(loop(loop(loop(loop(loop(x + y, y, x), y, x), y - 2, x), y, x), y - 2, x) + x, x - 2, 0)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (g5 x y) (- (- y 2) 2))))
