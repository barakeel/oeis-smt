;; sequence(s): A28214
;; terms: 1 36 820 15120 246736 3721536 53174080 730794240 9758653696 127501710336 1638112752640 20771546664960 260668843872256 3244288806567936 40111449573376000 493276560759521280 6039827824481468416 73691981266726158336 896516001844367196160
;; small program: loop(loop(loop(loop(2 * ((x + x) + x), x, loop(x + x, x, 2) - 1), 1, y) + (2 * ((2 * (x + x)) + x)), y, 1) + (2 * (2 * (x + x))), x, 1)
;; fast program: loop((loop(loop(loop((x + x) + x, x, loop(x + x, x, 2) - 1), 1, y) + (2 * (x + x)), y, 1) + (2 * (x + x))) + x, x, 1) * loop(x + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (h8 x) (- (v9 x) 1))))
...
(assert (forall ((x Int)) (= (fast x) (* (v5 x) (v10 x)))))
