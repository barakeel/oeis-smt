;; sequence(s): A26308
;; terms: 1 30 597 9974 151629 2176086 30060949 404288598 5331243357 69255757142 889174566501 11309336465622 142744102829485 1790283986115798 22334320279686453 277371931128111446
;; small program: loop((loop(loop2((loop((2 + y) * x, 2, x) - x) + y, y + y, y, 1, 2) + loop((2 + y) * x, 2, x), y, 1) + (2 * (x + x))) + x, x, 1)
;; fast program: loop((loop(loop((loop(loop(x + x, x, loop2(1 + (x * y), y, x, 1, 2 + (2 + 2))), 1, y) + (2 * (x + x))) + x, x, 1), 1, y) + ((((x + x) * 2) + x) * 2)) + x, x, 1)
(set-logic UFNIA)
