;; sequence(s): A28134
;; terms: 1 29 543 8401 117383 1545873 19643191 244174337 2994192135 36408715057 440432748119 5310970152033 63918578839207 768370553038481 9230114440859127 110830456328561089 1330457511950281799 15968985141074818545 191652591219028933015
;; small program: loop((loop(loop(loop(2 * ((x + x) + x), x, loop(x + x, x, 2) - 1), 1, y) + (2 * (x + x)), y, 1) + (2 * ((x + x) + x))) + x, x, 1)
;; fast program: loop((loop(loop((loop(loop((x + x) + x, x, loop(x + x, x, 2) - 1), 1, y) + x) + x, x, 1) * loop(x + x, x, 1), 1, y) + x) + ((x * (1 + 2)) * 2), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (+ (v6 x y) x) (* (* x (+ 1 2)) 2)))))
