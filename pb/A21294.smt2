;; sequence(s): A21294
;; terms: 1 23 368 5098 65559 806541 9640606 112964816 1304876837 14914020979 169097614764 1905464222454 21368620595635 238731453906137 2659135639187642 29548298847110812 327711548662770753
;; small program: loop((loop(loop(1 + (loop((2 + y) * x, 2, x) - x), y, 1) + loop((x + x) + x, 2, x), y, 1) + x) + x, x, 1)
;; fast program: loop((loop2(((2 * ((2 * (x + x)) + y)) - 1) + x, y + y, y, 1, 2) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (+ (- (* 2 (+ (* 2 (+ x x)) y)) 1) x))))
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (+ (w6 x y) (* 2 (+ (* 2 (+ x x)) x))) x))))
