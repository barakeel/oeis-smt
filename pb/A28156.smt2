;; sequence(s): A28156
;; terms: 1 32 653 10852 160329 2196744 28582501 358350284 4370826977 52208034736 613595259069 7120485417396 81801199876345 932195385723608 10554384751300757 118870719509813788 1333093245475954833
;; small program: loop(loop(loop(loop((x + x) + x, y + y, 1) + (loop((2 + y) * x, 2, x) - x), y, 1) + (2 * (x + x)), y, 1) + (2 * (2 * (x + x))), x, 1)
;; fast program: loop((loop(loop((loop(loop(((x - 1) + x) * (x * x), 1, loop(x + x, x, 1)), 1, y) + x) + (((x + x) * 2) * 2), x, 1), 1, y) + x) + ((((x + x) * 2) + x) * 2), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f9 x) (* (+ (- x 1) x) (* x x)))))
(assert (= g9 1))
