;; sequence(s): A28161
;; terms: 1 34 737 12998 203493 2949042 40498969 534679966 6850824365 85749662570 1053427606881 12745999413654 152297310673717 1800758893450018 21104410600012073 245479747499528462
;; small program: loop(loop(loop(loop((x + x) + x, y + y, 1) + (loop((2 + y) * x, 2, x) - x), y, 1) + (2 * ((2 * (x + x)) + x)), y, 1) + (2 * (x + x)), x, 1)
;; fast program: loop((loop((loop2((2 * ((2 * (x + x)) + x)) + (y * y), y + y, y, 1, 2) + (2 * (2 * (x + x)))) + x, y, 1) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (+ (* 2 (+ (* 2 (+ x x)) x)) (* y y)))))
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (+ (+ (w7 x y) (* 2 (* 2 (+ x x)))) x))))
