;; sequence(s): A112999
;; terms: 1 5 221 331997 24883531997 139314094387531997 82606411393217618227531997 6984964247224120535022357995827531997 109110688415578301444592123476429107940843827531997
;; small program: loop(loop(loop2(loop(x * y, y, x), y, x, 1, x), 1, 1 + y) + x, x, 1)
;; fast program: loop(loop((loop2(x * y, y, x - 2, 1, loop(x * y, x - 1, x)) * loop((x * x) * x, 1, loop(x * y, x, 1))) * loop2(x * y, y, x, 1 + x, 1 + x), 1, y) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (i10 x) (+ 1 x))))
(assert (forall ((x Int)) (= (j10 x) (+ 1 x))))
...
(assert (forall ((x Int)) (= (f5 x) (* (* (w6 x) (v8 x)) (w10 x)))))
(assert (= g5 1))
