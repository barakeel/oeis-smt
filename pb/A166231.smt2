;; sequence(s): A166231
;; terms: 1 4 48 1280 69632 7602176 1660944384 725849473024 634418209226752 1109011408239984640 3877275019992826380288 27111105978154633171828736 379138479844261543254652092416 10604214183559196000870296488771584
;; small program: loop(loop(x + x, y, x), x, 1) * loop2(2 * ((x - y) + x), x, x, 1, 1)
;; fast program: loop2(2 * (2 * (2 * (x - y))), x, x - 1, if x <= 0 then 1 else (2 + 2), 1) * loop2(x * y, y + y, x - 1, 1, 2)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (i3 x) (ite (<= x 0) 1 (+ 2 2)))))
(assert (= j3 1))
