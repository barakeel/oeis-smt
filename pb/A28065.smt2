;; sequence(s): A28065
;; terms: 1 26 437 6058 75525 882258 9876613 107390426 1143865349 12003195490 124572616389 1282173260394 13113918930373 133479729281522 1353536864252165 13685361486439162 138053939020888197
;; small program: loop(((loop(loop(loop((2 * (x + x)) + x, x, loop(x + x, x, 2) - 1), 1, y) + (2 * (2 * (x + x))), y, 1) + x) + x) + x, x, 1)
;; fast program: loop(loop(loop(((loop((loop(x + x, x, 2) - 1) * loop2(x * y, y, x, 1, 1 + (2 + 2)), 1, y) + x) + x) + x, x, 1), 1, y) + (((x + x) * 2) * 2), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (v6 x y) (* (* (+ x x) 2) 2)))))
