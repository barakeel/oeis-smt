;; sequence(s): A16268
;; terms: 1 24 421 6516 94297 1308720 17653357 233276172 3035108833 39015048456 496711748533 6274383820068 78744877055209 982913508020832 12212686913191549 151147215944655804 1864311294185799025
;; small program: loop((loop(1 + loop((2 + y) * x, 2, x), y, 1) - x) + loop((2 + y) * x, 2, x), x, 1)
;; fast program: loop((loop2(1 + (x * y), y, y, 1, 2 * (2 + (2 + 2))) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (+ (+ (w5 x y) (* 2 (+ (* 2 (+ x x)) x))) x))))
