;; sequence(s): A339487
;; terms: 8 192 3344 52800 806168 12166272 182871584 2744987520 41184460328 617815378752 9267473759024 139013323899840 2085205952447288 31278119775589632 469171949135641664 7037579999715797760 105563703809659391048 1583455576216052670912 23751833738601248135504 356277506555834959529280
;; small program: ((loop(((loop((loop(1 + ((loop(x * x, 2, 2) * x) - x), y, 1) + (2 * (x + x))) + x, y, 1) + x) + x) + x, x, 1) * 2) * 2) * 2
;; fast program: ((loop((loop(loop2(((2 * (x + x)) + x) + y, 1 + ((y + y) + y), x, 1, 2 + 2), 1, y) + (2 * ((2 * ((x + x) + x)) + x))) + x, x, 1) * 2) * 2) * 2
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (+ (+ (v5 x y) (* 2 (+ (* 2 (+ (+ x x) x)) x))) x))))
(assert (forall ((x Int)) (= (g4 x) x)))
(assert (= h4 1))
(assert (forall ((x Int) (y Int)) (= (u4 x y) (ite (<= x 0) y (f4 (u4 (- x 1) y) x)))))
(assert (forall ((x Int)) (= (v4 x) (u4 (g4 x) h4))))
(assert (forall ((x Int)) (= (fast x) (* (* (* (v4 x) 2) 2) 2))))
