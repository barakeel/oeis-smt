...
(assert (forall ((x Int)) (= (small x) (+ (+ (+ (+ (divf (+ (v0 x) x) 2) 1) x) x) x))))
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (+ (modf (- y (divf (divf x 2) (+ 1 2))) 2) x) x))))
(assert (= g2 2))
(assert (forall ((x Int)) (= (h2 x) x)))
(assert (forall ((x Int) (y Int)) (= (u2 x y) (ite (<= x 0) y (f2 (u2 (- x 1) y) x)))))
(assert (forall ((x Int)) (= (v2 x) (u2 g2 (h2 x)))))
(assert (forall ((x Int)) (= (fast x) (+ (divf (+ (+ (+ (+ (+ (divf (+ (+ (- (+ (+ (+ (+ (+ (v2 x) (ite (<= x 0) 1 x)) x) x) x) x) (ite (<= x 0) 1 2)) x) x) (+ 1 2)) 1) x) x) x) x) (ite (<= x 0) 1 2)) x))))
