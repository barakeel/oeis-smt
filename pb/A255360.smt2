;; sequence(s): A255360
;; terms: 1 1 263130836933693530167218012160000000
;; small program: loop(x * y, loop(x * x, 2, x) * x, 1)
;; fast program: loop(loop2(x * y, 1 + y, x, 1, 1 + x) * loop(x * y, x, 1), 1, loop(x * x, 1, ((x * x) - x) * x))
(set-logic UFNIA)
...
(assert (= g5 1))
(assert (forall ((x Int)) (= (h5 x) (* (- (* x x) x) x))))
