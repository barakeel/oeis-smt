;; sequence(s): A20579
;; terms: 1 23 355 4595 53851 592403 6240235 63710915 635468251 6225852083 60146237515 574587484835 5439634923451 51116555484563 477406092913195 4435981769620355 41041272503703451 378327871809737843
;; small program: loop(loop(loop((x + x) + x, y + y, 1) + (2 * ((x + x) + x)), y, 1) + (2 * (2 * (x + x))), x, 1)
;; fast program: loop((((((loop(loop2(x * y, y, 1 + x, 1, 1 + (2 * (2 + 2))) - loop((x * x) * x, 1, loop(x + x, x, 2)), 1, y) + x) + x) + x) + x) + x) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f3 x y) (+ (+ (+ (+ (+ (+ (v4 x y) x) x) x) x) x) x))))
