;; sequence(s): A20983
;; terms: 1 31 643 11155 174811 2566291 36012523 489103555 6481822171 84295081651 1080159920203 13679489505955 171612008243131 2136467306462611 26431716545456683 325327578356628355 3987253758579873691 48696950467661485171 593012553894264829963
;; small program: loop(loop(loop((x + x) + x, y + y, 1) + (2 * ((2 * (x + x)) + x)), y, 1) + loop((2 + y) * x, 2, x), x, 1)
;; fast program: loop(loop(loop2((((y * y) + x) + x) + x, y + y, x, 1, 2) * loop2(x * y, y, x, 1, 1 + 2), 1, y) + (2 * ((2 * (x + x)) + x)), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (+ (+ (+ (* y y) x) x) x))))
