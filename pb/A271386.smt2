;; sequence(s): A271386
;; terms: 1 1 36 291600 1851776640000 23813032808678400000000 1333916640950593574375424000000000000 618764594221522786972353235328676003840000000000000000
;; small program: loop(x * x, 1, loop(loop(((2 * (x * y)) - x) * y, y, x), x, 1))
;; fast program: loop(x * x, 1, loop(loop(((2 * (x * y)) - x) * y, y, 1) * x, x, 1))
(set-logic UFNIA)
...
(assert (= g0 1))
(assert (forall ((x Int) (y Int)) (= (f2 x y) (* (- (* 2 (* x y)) x) y))))
...
(assert (= g3 1))
(assert (forall ((x Int) (y Int)) (= (f5 x y) (* (- (* 2 (* x y)) x) y))))
