...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ (+ (modf (+ (divf (* y y) (+ 2 (* 2 (+ 2 2)))) x) 2) 1) x))))
(assert (forall ((x Int)) (= (g0 x) (+ x x))))
(assert (= h0 2))
(assert (forall ((x Int) (y Int)) (= (u0 x y) (ite (<= x 0) y (f0 (u0 (- x 1) y) x)))))
(assert (forall ((x Int)) (= (v0 x) (u0 (g0 x) h0))))
(assert (forall ((x Int)) (= (small x) (+ (+ (- (v0 x) 1) x) x))))
(assert (forall ((x Int)) (= (fast x) (+ (+ (divf (+ (* (* (+ (+ (+ (+ (divf (- (ite (<= x 0) 1 x) 1) (+ 1 (+ 2 2))) x) 1) (divf x (+ 1 (+ 2 2)))) x) 2) 2) 1) (+ 1 2)) x) x))))
