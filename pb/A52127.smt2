;; sequence(s): A52127
;; terms: 1 1 8 96 2112 68160 3087360 185633280 14301020160 1372232171520 160390869811200 22426206024499200 3695148753459609600 708443854690399027200 156340439420689081958400 39342248735234589720576000 11197266840049016358567936000
;; small program: loop(loop(loop((x * y) + loop(0 - (x + x), y, 1), y, 1) + (x * y), y, 1) + (x * y), x, 1) * loop(x * y, x, 1)
;; fast program: loop(loop(loop(loop2(x * y, y, y, 1, 0 - 2) + (x * y), y, 1) + (x * y), y, 1) + (x * y), x, 1) * loop(x * y, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (- 0 (+ x x)))))
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (+ (w8 x y) (* x y)))))
