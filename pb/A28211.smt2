;; sequence(s): A28211
;; terms: 1 34 729 12614 192605 2711058 36061873 460391518 5699774949 68923376522 818303379257 9575924656662 110774806146733 1269639423098626 14443439746998081 163313818986599246
;; small program: loop(loop(loop(loop((x + x) + x, y + y, 1) + (loop((2 + y) * x, 2, x) - x), y, 1) + (2 * ((x + x) + x)), y, 1) + (2 * (2 * (x + x))), x, 1)
;; fast program: loop((loop(loop(loop(loop2(((x + x) + x) + y, y + y, x, 1, 2) * loop2(x * y, y, x, 1, 1 + 2), 1, y) + (2 * (2 * (x + x))), x, 1), 1, y) + x) + ((((x + x) * 2) + x) * 2), x, 1)
(set-logic UFNIA)
