;; sequence(s): A20494
;; terms: 1 24 391 5394 67921 808164 9257851 103242534 1128792181 12157176504 129404822911 1364594578074 14281107062041 148528742855244 1536752582181571 15830742722056014 162476531910613501
;; small program: loop(loop(loop((2 * (x + x)) + x, x, loop(x + x, x, 2) - 1), 1, y) + loop((x + x) + x, 2, x), x, 1)
;; fast program: loop((loop((loop(x + x, x, 2) - 1) * loop2(x * y, y, x, 1, 1 + (2 + 2)), 1, y) + (2 * (2 * (x + x)))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f6 x) (* (- (v7 x) 1) (w8 x)))))
(assert (= g6 1))
