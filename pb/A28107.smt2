;; sequence(s): A28107
;; terms: 1 35 790 14630 242011 3723545 54492880 769001660 10558544821 141919432655 1875649181770 24453637571090 315271616180431 4027179810711365 51043597496835460 642723844104282920
;; small program: loop(((loop(loop(loop((x + x) + x, y + y, 1) + (loop((2 + y) * x, 2, x) - x), y, 1) + loop((2 + y) * x, 2, x), y, 1) + x) + x) + x, x, 1)
;; fast program: loop((loop(loop2((2 * (x + x)) + y, 1 + ((y + y) + y), x, 1, 2 + 2) * loop2(x * y, y, x, 1, 1 + 2), 1, y) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f8 x y) (+ (* 2 (+ x x)) y))))
(assert (forall ((x Int) (y Int)) (= (g8 x y) (+ 1 (+ (+ y y) y)))))
