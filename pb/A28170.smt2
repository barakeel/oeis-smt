;; sequence(s): A28170
;; terms: 1 28 495 7070 89201 1038408 11432275 120872290 1239829701 12425000588 122271240455 1185943687110 11369353359001 107963322279568 1017222704311035 9522095291175530 88651748959581101
;; small program: loop((loop(loop(loop((x + x) + x, y + y, 1) + (2 * ((x + x) + x)), y, 1) + (2 * (2 * (x + x))), y, 1) + (2 * (x + x))) + x, x, 1)
;; fast program: loop((loop((loop2((2 * ((x + x) + x)) + ((y * y) * y), y + y, y, 1, 2) + (2 * (x + x))) + x, y, 1) + (2 * (2 * (x + x)))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (+ (* 2 (+ (+ x x) x)) (* (* y y) y)))))
