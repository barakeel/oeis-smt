;; sequence(s): A53808
;; terms: 1 5 15 36 76 148 273 485 839 1424 2384 3952 6505 10653 17383 28292 45964 74580 120905 195885
;; small program: loop(loop(loop2(2 + (x + y), x, y, 1, 0) + x, y, x), x, 1) + x
;; fast program: loop((loop((loop2(x + y, x, x, 1 + (2 + 2), 1 + 2) - 1) - ((2 + x) * x), 1, 1 + x) - 2) - 2, 1, 2 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f4 x) (- (- (w5 x) 1) (* (+ 2 x) x)))))
(assert (= g4 1))
