;; sequence(s): A28196
;; terms: 1 35 776 13930 221151 3240825 44906926 597218720 7698198101 96837355615 1194615734676 14505367659510 173840151316651 2060770407338405 24205227062904026 282088240414000300 3265447176464616801 37582206965366853195 430364865022642734976
;; small program: loop(loop(loop(loop((2 * (x + x)) + x, x, loop(x + x, x, 2) - 1), 1, y) + (loop((2 + y) * x, 2, x) - x), y, 1) + loop((x + x) + x, 2, x), x, 1)
;; fast program: loop((loop(loop((loop((loop(x + x, x, 2) - 1) * loop2(x * y, y, x, 1, 1 + (2 + 2)), 1, y) + x) + (((x * 2) * 2) * 2), x, 1), 1, y) + x) + ((((x + x) * 2) + x) * 2), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f10 x) (* (- (v11 x) 1) (w12 x)))))
(assert (= g10 1))
...
(assert (forall ((x Int) (y Int)) (= (f9 x y) (+ (+ (v10 x y) x) (* (* (* x 2) 2) 2)))))
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (+ (+ (v8 x y) x) (* (+ (* (+ x x) 2) x) 2)))))
