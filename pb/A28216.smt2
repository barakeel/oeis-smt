;; sequence(s): A28216
;; terms: 1 36 817 14952 241213 3582348 50196889 673865424 8755297045 110878147380 1375697840881 16786476031416 202032169207597 2403762666000732 28323951022216393 331005811232778528
;; small program: loop(loop(loop(loop((x + x) + x, y + y, 1) + (loop((2 + y) * x, 2, x) - x), y, 1) + (2 * ((x + x) + x)), y, 1) + (2 * ((2 * (x + x)) + x)), x, 1)
;; fast program: loop((loop(loop(loop(loop2(((x + x) + x) + y, y + y, x, 1, 2) * loop2(x * y, y, x, 1, 1 + 2), 1, y) + (2 * ((2 * (x + x)) + x)), x, 1), 1, y) + ((((x + x) * 2) + x) * 2)) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f8 x) (* (w9 x) (w10 x)))))
(assert (= g8 1))
