;; sequence(s): A26024
;; terms: 1 26 443 6280 80481 969126 11196103 125634740 1380061661 14917299826 159239422563 1683073423200 17647253908441 183823250672126 1904399083525823 19639540889107660 201755613688828821
;; small program: loop((loop(loop(loop((2 * (x + x)) + x, x, loop(x + x, x, 2) - 1), 1, y) + loop((x + x) + x, 2, x), y, 1) + x) + x, x, 1)
;; fast program: loop((loop(loop((loop((loop(x + x, x, 2) - 1) * loop2(x * y, y, x, 1, 1 + (2 + 2)), 1, y) + x) + x, x, 1), 1, y) + x) + (((x + x) * 2) * 2), x, 1)
(set-logic UFNIA)
