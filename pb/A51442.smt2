;; sequence(s): A51442
;; terms: 1 3 17 145 1649 23401 397585 7861953 177264449 4486784401 125937424601 3881436747409 130291290501553 4731091158953433 184761021583202849 7721329860319737601 343809097055019694337 16248996011806421522977
;; small program: loop2((x * y) + x, y, x, 1, x) + loop2(x * y, y, x, x, x)
;; fast program: (loop2(x * y, y, x - 1, 1, 1 + x) * (1 + x)) + (((x * x) * x) * loop2(x * y, y, x - 2, 1, x))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (+ (* (w2 x) (+ 1 x)) (* (* (* x x) x) (w3 x))))))
