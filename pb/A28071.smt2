;; sequence(s): A28071
;; terms: 1 29 548 8590 121551 1615119 20584858 254810960 3087662501 36816560209 433529214768 5054413691730 58455083456251 671574295857299 7673016284430278 87260245701154900 988433749842078801
;; small program: loop(((loop(loop(loop((2 * (x + x)) + x, x, loop(x + x, x, 2) - 1), 1, y) + (loop((2 + y) * x, 2, x) - x), y, 1) + x) + x) + x, x, 1)
;; fast program: loop((loop(loop(((loop((loop(x + x, x, 2) - 1) * loop2(x * y, y, x, 1, 1 + (2 + 2)), 1, y) + x) + x) + x, x, 1), 1, y) + ((((x + x) * 2) + x) * 2)) + x, x, 1)
(set-logic UFNIA)
