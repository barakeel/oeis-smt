;; sequence(s): A33811
;; terms: 3 10 24 49 92 164 283 478 796 1313 2152 3512 5715 9282 15056 24401 39524 63996 103595 167670
;; small program: loop(loop(loop2(x + y, x, y, 1, 2) + x, y, x), 1 + x, 0)
;; fast program: loop((loop2(x + y, x, x, 1 + (2 + (2 + 2)), 2 + 2) - 1) - (2 * (2 * (2 + x))), 1, 2 + x) + 2
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (- (- (w4 x) 1) (* 2 (* 2 (+ 2 x)))))))
(assert (= g3 1))
