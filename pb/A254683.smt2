;; sequence(s): A254683
;; terms: 1 69 1064 8736 49350 216342 787968 2489448 7024407 18074875 43072848 96186272 203145852 408774588 788378400 1464523344 2631173181 4587701601 7785938104 12894168000
;; small program: loop(loop(loop(loop(loop(((loop(x * x, 2, y) * y) * y) + x, y, x), y, x), y, x), 1 + y, x), x, 1)
;; fast program: loop(loop(loop(loop(loop(loop((x * x) * x, 1, y * y) + x, y, x), y, x), y, x), 1 + y, x), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (+ (* (* (v5 x y) y) y) x))))
...
(assert (= g11 1))
(assert (forall ((x Int) (y Int)) (= (h11 x y) (* y y))))
