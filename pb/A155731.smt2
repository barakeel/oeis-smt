;; sequence(s): A155731
;; terms: 1 3 30 810004 348687049529230979905036801029
;; small program: loop(1 + (loop2(x * y, y, y, x, x) + y), x, 1)
;; fast program: loop((loop2(x * y, y, y - 2, 1, x) * ((x * x) * x)) + (1 + y), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f0 x y) (+ 1 (+ (w1 x y) y)))))
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (* (w3 x y) (* (* x x) x)) (+ 1 y)))))
