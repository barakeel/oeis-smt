;; sequence(s): A213776
;; terms: 1 8 30 81 184 376 717 1304 2294 3941 6656 11104 18361 30168 49342 80441 130840 212472 344645 558600
;; small program: loop(loop(loop(loop2(x + y, x, y, 2 + 2, 1) + x, y, x), y, x) + y, x, 1) + x
;; fast program: loop(loop(((loop((((((((((loop2(x + y, x, x, 1 + (2 * (2 + 2)), 1 + (2 + 2)) - 2) - 2) - 2) - x) - x) - x) - 1) - x) - x) - x, 1, 2 + x) - 2) - x) - x, 1, y) + x, x, 1) - x
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f6 x) (- (- (- (- (- (- (- (- (- (- (w7 x) 2) 2) 2) x) x) x) 1) x) x) x))))
(assert (= g6 1))
(assert (forall ((x Int)) (= (h6 x) (+ 2 x))))
(assert (forall ((x Int) (y Int)) (= (u6 x y) (ite (<= x 0) y (f6 (u6 (- x 1) y))))))
(assert (forall ((x Int)) (= (v6 x) (u6 g6 (h6 x)))))
(assert (forall ((x Int)) (= (f5 x) (- (- (- (v6 x) 2) x) x))))
(assert (= g5 1))
