;; sequence(s): A13797
;; terms: 1728 35831808 743008370688 15407021574586368 319479999370622926848 6624737266949237011120128 137370551967459378662586974208 2848515765597237675947403497177088 59066822915424320448445358917464096768
;; small program: loop(loop((2 + y) * x, 2, x), loop(1 + (x + x), 2, x), 1)
;; fast program: (loop(x * x, 1, loop2(x * y, y, x, 1, 2 * (2 + (2 + 2)))) * (2 * (2 + (2 + 2)))) * loop(x * x, 1, loop2(x * y, y, 1 + x, 1, 2 * (2 + (2 + 2))))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (* (v3 x) (* 2 (+ 2 (+ 2 2)))) (v5 x)))))
