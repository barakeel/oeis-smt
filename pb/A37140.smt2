;; sequence(s): A37140
;; terms: 5 18 44 91 172 308 533 902 1504 2483 4072 6648 10821 17578 28516 46219 74868 121228 196245 317630
;; small program: loop(loop(loop2(x + y, x, 1 + y, 2, 1) + x, y, x), 1 + x, 0)
;; fast program: (((loop(((((loop2(x + y, x, x, 1 + (2 * (2 + (2 + 2))), 2 * (2 + 2)) - 2) - (2 * (2 * (2 + x)))) - x) - x) - x, 1, 2 + x) - 2) - 2) - x) - 1
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (- (- (- (- (- (w4 x) 2) (* 2 (* 2 (+ 2 x)))) x) x) x))))
(assert (= g3 1))
(assert (forall ((x Int)) (= (h3 x) (+ 2 x))))
(assert (forall ((x Int) (y Int)) (= (u3 x y) (ite (<= x 0) y (f3 (u3 (- x 1) y))))))
(assert (forall ((x Int)) (= (v3 x) (u3 g3 (h3 x)))))
(assert (forall ((x Int)) (= (fast x) (- (- (- (- (v3 x) 2) 2) x) 1))))
