;; sequence(s): A135426
;; terms: 1 56 200704 46036680704 675821419082285056 634950507820721016852709376 38179283319360245979311522664775942144 146924980824268039790798000267293196637757037871104
;; small program: loop(2 * (2 * (x + x)), x * x, loop((2 * ((x + x) + x)) + x, x, 1))
;; fast program: (loop2(x * y, y, x, 1, 1 + (2 + (2 + 2))) * loop2(x * y, y, x, 1, loop(x + x, x - 1, 2))) * loop(x * x, 1, loop2(x * y, y, x, 1, loop(x + x, x, 1)))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (* (w2 x) (w3 x)) (v5 x)))))
