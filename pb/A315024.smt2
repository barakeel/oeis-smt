...
(assert (= v0 (u0 g0 h0)))
(assert (forall ((x Int)) (= (small x) (+ (divf (+ (+ (* (+ (+ (divf (+ 1 (+ (divf x v0) x)) (ite (<= x 0) 1 2)) x) x) (+ 2 2)) 1) x) (+ 1 2)) x))))
(assert (forall ((x Int)) (= (fast x) (+ (divf (+ (+ (* (+ (+ (divf (+ 1 (+ (divf x (+ 1 (+ 2 (* 2 (+ 2 2))))) x)) (ite (<= x 0) 1 2)) x) x) (+ 2 2)) 1) x) (+ 1 2)) x))))
