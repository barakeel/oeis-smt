;; sequence(s): A241032
;; terms: 8 72 1300 31752 872644 25170552 741453700 22051219752 658764967684 19722455410392 591076720682500 17723450167663752 531571748759349124 15945186209351359032 478326193010497869700 14349345894391097803752
;; small program: ((1 + loop((x + x) + x, x, 1)) * (1 + loop((2 * (x + x)) + x, x, 1))) * (1 + loop(x + x, x, 1))
;; fast program: ((1 + loop2(x * y, y, x, 1, 1 + (2 + 2))) * (1 + loop(x + x, x, 1))) * (1 + loop2(x * y, y, x, 1, 1 + 2))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (small x) (* (* (+ 1 (v0 x)) (+ 1 (v1 x))) (+ 1 (v2 x))))))
...
(assert (forall ((x Int)) (= (fast x) (* (* (+ 1 (w3 x)) (+ 1 (v4 x))) (+ 1 (w5 x))))))
