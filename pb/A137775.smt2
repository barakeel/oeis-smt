;; sequence(s): A137775
;; terms: 1 0 3 6 45 252 1935 16146 153657 1616760 18699579 235498590 3207570597 46968796404 735689606535 12272343940458 217191191400945 4064131571557104 80166987477918963 1662468879466624950
;; small program: loop(loop(loop((x * y) + loop(0 - ((x + x) + x), y, 1), y, 1) + (x * y), y, 1) + (x * y), x, 1)
;; fast program: loop(loop(loop(loop2(x * y, y, y, 1, 1 - (2 + 2)) + (x * y), y, 1) + (x * y), y, 1) + (x * y), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (- 0 (+ (+ x x) x)))))
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (+ (w7 x y) (* x y)))))
