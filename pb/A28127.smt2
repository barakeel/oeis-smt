;; sequence(s): A28127
;; terms: 1 30 581 9270 132861 1781910 22867141 284524950 3461329421 41398890390 488675095701 5708526430230 66125948101981 760712837896470 8701144748444261 99045775841641110 1122836116105178541
;; small program: loop(loop(loop(loop((2 * (x + x)) + x, x, loop(x + x, x, 2) - 1), 1, y) + (2 * (x + x)), y, 1) + (loop((2 + y) * x, 2, x) - x), x, 1)
;; fast program: loop((loop(loop(loop((loop(x + x, x, 2) - 1) * loop2(x * y, y, x, 1, 1 + (2 + 2)), 1, y) + (2 * (x + x)), x, 1), 1, y) + x) + ((((x * 2) * 2) + x) * 2), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f8 x y) (+ (v9 x y) (* 2 (+ x x))))))
