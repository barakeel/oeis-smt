;; sequence(s): A271946
;; terms: 1 720 344881152000 2208058019165981638656000000 1369986068925795885347091500568179543900160000000000 363392722685428853076589064611759104109572860599125858715484081356800000000000000000
;; small program: loop(loop(x * y, 2 * ((y + y) + y), x), x, 1)
;; fast program: loop(loop(loop2(x * y, 1 + y, x, 1, 1 + x) * loop(x * y, x, 1), 1, loop((x + x) + x, 1, y)) * x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (h3 x y) (v6 x y))))
