;; sequence(s): A28021
;; terms: 1 32 665 11380 174261 2483592 33669985 440024060 5592152621 69530670352 849490297305 10231269580740 121775852180581 1435121355116312 16771617736444625 194605340572369420
;; small program: loop((loop(loop(loop((x + x) + x, y + y, 1) + (loop((2 + y) * x, 2, x) - x), y, 1) + (2 * ((2 * (x + x)) + x)), y, 1) + x) + x, x, 1)
;; fast program: loop((loop(loop((loop(loop2(1 + (x * y), y, x, 1, 1 + (2 + 2)) * loop(x + x, x, 1), 1, y) + x) + (((x * 2) * 2) * 2), x, 1), 1, y) + x) + ((((x + x) * 2) + x) * 2), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (+ (+ (v8 x y) x) (* (* (* x 2) 2) 2)))))
