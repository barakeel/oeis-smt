;; sequence(s): A28077
;; terms: 1 27 472 6822 88963 1091349 12886714 148484664 1683799645 18895210191 210582084076 2336271745626 25842423980647 285296827032153 3145632963614158 34654478019199308 381572508324065569
;; small program: loop((loop(loop(loop((x + x) + x, x, loop(x + x, x, 2) - 1), 1, y) + (loop((2 + y) * x, 2, x) - x), y, 1) + (2 * ((x + x) + x))) + x, x, 1)
;; fast program: loop((loop((loop(loop((x + x) + x, x, loop(x + x, x, 2) - 1), 1, y) + (2 * ((x + x) + x))) + x, y, 1) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
(set-logic UFNIA)
