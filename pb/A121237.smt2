;; sequence(s): A121237
;; terms: 1 257 7625597485244 340282366920938463463374615057365696700 2350988701644575015937473074444491355637331113544515325384424351020297893524511711399825
;; small program: loop(loop(loop2((x * y) + x, y, loop((x * x) * x, 1, 1 + x), 1, x), 1, y) + x, x, 1)
;; fast program: loop(loop(loop2(loop2(x * y, y, y - 1, 1, x) * (x * x), y, 1 + 2, 1 + x, x), 1, y) + x, x, 1)
(set-logic UFNIA)
