;; sequence(s): A21864
;; terms: 1 21 294 3466 37275 379407 3727648 35761572 337430709 3146261833 29083452762 267126341118 2441786303503 22239710844099 202003194298836 1830950757258904 16569016601938857 149754390830203005
;; small program: loop((loop(loop(1 + (2 * (x + x)), y, 1) + loop((x + x) + x, 2, x), y, 1) + (2 * ((x + x) + x))) + x, x, 1)
;; fast program: loop((loop(loop2(((2 * ((x + x) + x)) + x) + y, 1 + (2 * (y + y)), x, 1, 1 + (2 + 2)), 1, y) + (2 * (2 * (x + x)))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (+ (+ (* 2 (+ (+ x x) x)) x) y))))
