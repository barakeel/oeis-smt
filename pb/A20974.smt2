;; sequence(s): A20974
;; terms: 1 29 567 9337 139775 1971417 26715823 352001609 4543901439 57765992185 725866130639 9039113138601 111770394659743 1374351994892633 16823974751541615 205209952708309513 2495775222328385087 30282093196741317561 366714652062500686351
;; small program: loop((loop(loop(2 * ((2 * (x + x)) + x), y, 1) + loop((2 + y) * x, 2, x), y, 1) + (2 * ((x + x) + x))) + x, x, 1)
;; fast program: loop((loop(loop2((2 * ((x + x) + x)) + y, (2 * (y + y)) + y, x, 1, 1 + (2 + 2)) * loop(x + x, x, 1), 1, y) + x) + (((x * 2) + x) * 2), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f4 x y) (+ (+ (v5 x y) x) (* (+ (* x 2) x) 2)))))
