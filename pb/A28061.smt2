;; sequence(s): A28061
;; terms: 1 25 404 5390 64791 731595 7939714 83955520 872430581 8959286765 91262059824 924426748050 9327520083571 93861018423535 942726158964734 9456149685174980 94764094925599761 949061042927869905 9500560621143838444 95075305236466788310
;; small program: loop(((loop((loop(loop((2 * (x + x)) + x, x, loop(x + x, x, 2) - 1), 1, y) + (2 * ((x + x) + x))) + x, y, 1) + x) + x) + x, x, 1)
;; fast program: loop((loop(loop(((loop((loop(x + x, x, 2) - 1) * loop2(x * y, y, x, 1, 1 + (2 + 2)), 1, y) + x) + x) + x, x, 1), 1, y) + x) + (((x + x) + x) * 2), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f5 x y) (+ (+ (v6 x y) x) (* (+ (+ x x) x) 2)))))
