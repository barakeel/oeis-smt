;; sequence(s): A54374
;; terms: 1 32 55296 7247757312 92771293593600000 141830962344853556428800000 30619440571316366848044102687129600000 1077325790213073725701226681195621188514296627200000
;; small program: loop(loop(loop2((2 * (2 * (x + x))) * y, y, x, 1 + x, 1 + x), 1, y) * x, x, 1)
;; fast program: (loop(loop(loop2(x * y, y, x, 1 + x, 1 + x), 1, y) * x, x, 1) * loop2(x * y, y + y, x, 1, 2)) * loop2(x * y, y, x, 1, loop(x + x, x, 2))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (fast x) (* (* (v3 x) (w6 x)) (w7 x)))))
