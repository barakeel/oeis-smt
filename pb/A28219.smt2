;; sequence(s): A28219
;; terms: 1 39 961 19131 336217 5446035 83308177 1221791547 17352006793 240304555491 3261449180353 43542585627723 573464912457529 7467052092622707 96294712139682289 1231626797709018459
;; small program: loop(loop(loop(loop(2 * ((x + x) + x), x, loop(x + x, x, 2) - 1), 1, y) + (loop((2 + y) * x, 2, x) - x), y, 1) + (2 * ((2 * (x + x)) + x)), x, 1)
;; fast program: loop((loop(loop((loop(loop((x + x) + x, x, loop(x + x, x, 2) - 1), 1, y) + (2 * (x + x))) + x, x, 1) * loop(x + x, x, 1), 1, y) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
(set-logic UFNIA)
