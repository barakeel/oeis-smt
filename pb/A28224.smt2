;; sequence(s): A28224
;; terms: 1 38 911 17632 301245 4745706 70660507 1009770884 13991407529 189303883054 2513736124263 32882696484456 424941717543253 5436863390984882 68985988087521779 869260000449137548
;; small program: loop((loop(loop2((loop((2 + y) * x, 2, x) - x) + ((y * y) * y), y + y, y, 1, 2) + loop((2 + y) * x, 2, x), y, 1) + (2 * ((x + x) + x))) + x, x, 1)
;; fast program: loop((loop(loop((loop(loop2(((x + x) + x) + y, y + y, x, 1, 2) * loop(x * x, 1, loop(x + x, x, 1)), 1, y) + (2 * ((x + x) + x))) + x, x, 1), 1, y) + ((((x + x) * 2) + x) * 2)) + x, x, 1)
(set-logic UFNIA)
