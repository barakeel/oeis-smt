;; sequence(s): A25009
;; terms: 1 31 632 10682 162183 2297253 31000054 403684144 5115788645 63462153755 773891110356 9306180096486 110619493115587 1302189143870737 15203564170082738 176265669269562908
;; small program: loop((loop(loop(1 + (2 * ((2 * (x + x)) + x)), y, 1) + loop((x + x) + x, 2, x), y, 1) - x) + loop((2 + y) * x, 2, x), x, 1)
;; fast program: loop((loop(loop((loop2(1 + (x * y), y, y, 1, 2 + (2 * (2 + 2))) + (2 * (2 * (x + x)))) + x, x, 1), 1, y) + x) + ((((x + x) * 2) + x) * 2), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (+ (+ (w8 x y) (* 2 (* 2 (+ x x)))) x))))
