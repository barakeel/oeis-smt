;; sequence(s): A28139
;; terms: 1 29 537 8113 109193 1365777 16254169 186716321 2089642665 22928450545 247752021881 2644807869249 27960164339017 293249462073233 3055559544501273 31664588166403297 326635270896335849
;; small program: loop(loop(loop(loop((x + x) + x, y + y, 1) + (2 * ((x + x) + x)), y, 1) + (2 * ((2 * (x + x)) + x)), y, 1) + (2 * (x + x)), x, 1)
;; fast program: loop((loop(loop((loop2(((x + x) + x) + y, y + y, y, 1, 2) + (2 * (x + x))) + x, x, 1) * loop(x + x, x, 1), 1, y) + (2 * (2 * (x + x)))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (+ (+ (+ x x) x) y))))
