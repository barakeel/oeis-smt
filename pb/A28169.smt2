;; sequence(s): A28169
;; terms: 1 30 577 9114 129349 1723086 22075129 275977218 3397380877 41418850902 501892142401 6058699871562 72968197588885 877537225626078 10544318356835593 126631590349302546 1520293638382550173
;; small program: loop((loop((loop(loop(2 * ((x + x) + x), x, loop(x + x, x, 2) - 1), 1, y) + (2 * ((x + x) + x))) + x, y, 1) + (2 * (x + x))) + x, x, 1)
;; fast program: loop((loop(loop((loop((loop(x + x, x, 2) - 1) * loop2(x * y, y, x, 1, 2 + (2 + 2)), 1, y) + (2 * (x + x))) + x, x, 1), 1, y) + x) + (((x + x) + x) * 2), x, 1)
(set-logic UFNIA)
