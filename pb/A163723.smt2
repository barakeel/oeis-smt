;; sequence(s): A163723
;; terms: 1 7 21 49 101 193 351 617 1059 1787 2979 4923 8085 13219 21545 35037 56889 92269 149539 242229
;; small program: loop(loop(loop2(2 + (x + y), x, y, 2, 0) + x, y, 2) + x, x, 1)
;; fast program: loop(((((((loop2(x + y, x, x, 1 + (2 + 2), 1 + 2) - 2) - 2) * 2) - 1) - x) - x) - ((1 + x) * (2 + x)), 1, 2 + x)
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (f3 x) (- (- (- (- (* (- (- (w4 x) 2) 2) 2) 1) x) x) (* (+ 1 x) (+ 2 x))))))
(assert (= g3 1))
