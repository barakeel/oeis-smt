;; sequence(s): A28112
;; terms: 1 25 401 5285 62601 696045 7442401 77640805 797399801 8108665565 81939294801 824736146325 8280574247401 83010854259085 831368657137601 8321395551579845 83260955156605401 832895295388684605
;; small program: loop(loop(loop(loop((2 * (x + x)) + x, x, loop(x + x, x, 2) - 1), 1, y) + (2 * (x + x)), y, 1) + (2 * ((x + x) + x)), x, 1)
;; fast program: loop(loop(loop(loop((loop(x + x, x, 2) - 1) * loop2(x * y, y, x, 1, 1 + (2 + 2)), 1, y) + (2 * (x + x)), x, 1), 1, y) + (2 * ((x + x) + x)), x, 1)
(set-logic UFNIA)
