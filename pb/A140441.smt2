;; sequence(s): A140441
;; terms: 1 1 1 1 3 221
;; small program: (loop(loop((1 + (x * y)) + y, x - 2, y), x, 0) + 1) - x
;; fast program: loop((loop((x * x) * x, 1, loop(x * y, x, 1)) + 1) + (y * y), (x - 2) - 1, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f2 x y) (+ (+ (v3 x) 1) (* y y)))))
(assert (forall ((x Int)) (= (g2 x) (- (- x 2) 1))))
(assert (= h2 1))
