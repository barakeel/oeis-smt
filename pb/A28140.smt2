;; sequence(s): A28140
;; terms: 1 30 577 9090 128053 1682430 21103129 256309050 3041879005 35492692950 408887138881 4665046178130 52827593605957 594751348348590 6665329755603433 74427620033028330 828693209595614509
;; small program: loop(loop(loop(loop((x + x) + x, y + y, 1) + (loop((2 + y) * x, 2, x) - x), y, 1) + (2 * ((x + x) + x)), y, 1) + (2 * (x + x)), x, 1)
;; fast program: loop((loop((loop2((2 * ((x + x) + x)) + (y * y), y + y, y, 1, 2) + (2 * (2 * (x + x)))) + x, y, 1) + ((((x + x) * 2) + x) * 2)) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (+ (* 2 (+ (+ x x) x)) (* y y)))))
