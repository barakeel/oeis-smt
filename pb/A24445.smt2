;; sequence(s): A24445
;; terms: 1 30 597 9934 149709 2121126 28836949 380838558 4924739997 62690732182 788556862821 9827670001422 121598064661165 1495950059553798 18319924811095413 223529877519404926 2719305099847790013 33001398296589107574 399716050359089793925
;; small program: loop((loop(loop(1 + (2 * ((2 * (x + x)) + x)), y, 1) + loop((2 + y) * x, 2, x), y, 1) + (2 * ((x + x) + x))) + x, x, 1)
;; fast program: loop(loop(loop2((2 * ((2 * (x + x)) + x)) + y, 1 + ((2 * ((y + y) + y)) + y), x, 1, 2 * (2 + 2)), 1, y) + (2 * (2 * ((x + x) + x))), x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f6 x y) (+ (* 2 (+ (* 2 (+ x x)) x)) y))))
