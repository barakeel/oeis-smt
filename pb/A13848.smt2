;; sequence(s): A13848
;; terms: 512 16777216 549755813888 18014398509481984 590295810358705651712 19342813113834066795298816 633825300114114700748351602688 20769187434139310514121985316880384 680564733841876926926749214863536422912
;; small program: loop(x + x, (2 * (2 * (2 + (2 * (x + x))))) - x, 2)
;; fast program: loop((x * x) * x, 1, loop(x * x, 1, loop(x + x, x, 2))) * loop((x * x) * x, 1, loop2(x * y, y, x, 2, 2 * (2 + 2)))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (g0 x) (- (* 2 (* 2 (+ 2 (* 2 (+ x x))))) x))))
(assert (= h0 2))
