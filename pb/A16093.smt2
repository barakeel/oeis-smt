;; sequence(s): A16093
;; terms: 1 41 1055 21805 395871 6595701 103397575 1549123085 22414242191 315506452261 4343673465495 58723278269565 781995624847711 10282227616659221 133750220517219815 1723860289008683245
;; small program: loop(loop(loop2((loop((2 + y) * x, 2, x) - x) + ((y * y) * y), y + y, y, 1, 2) + (2 * ((2 * (x + x)) + x)), y, 1) + loop((2 + y) * x, 2, x), x, 1)
;; fast program: loop((loop(loop((loop2((2 * ((x + x) + x)) + (y * y), y + y, y, 1, 2) + (2 * (x + x))) + x, x, 1) * loop(x + x, x, 1), 1, y) + (2 * ((2 * (x + x)) + x))) + x, x, 1)
(set-logic UFNIA)
...
(assert (forall ((x Int) (y Int)) (= (f8 x y) (+ (* 2 (+ (+ x x) x)) (* y y)))))
...
(assert (forall ((x Int) (y Int)) (= (f7 x y) (+ (+ (w8 x y) (* 2 (+ x x))) x))))
