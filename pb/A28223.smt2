;; sequence(s): A28223
;; terms: 1 37 863 16241 269703 4129041 59748151 829986817 11183795975 147236359985 1903757010519 24269169222753 305923748441767 3821741982426769 47397910610955767 584393237595955649
;; small program: loop((loop(loop2((2 * ((2 * (x + x)) + x)) + ((y * y) * y), y + y, y, 1, 2) + loop((2 + y) * x, 2, x), y, 1) + (2 * ((x + x) + x))) + x, x, 1)
;; fast program: loop((loop(loop((loop2((2 * ((x + x) + x)) + (y * y), y + y, y, 1, 2) + (2 * (x + x))) + x, x, 1) * loop(x + x, x, 1), 1, y) + (2 * ((x + x) + x))) + x, x, 1)
(set-logic UFNIA)
