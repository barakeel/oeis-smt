;; sequence(s): A74542
;; terms: 3 16 104 736 5408 40576 308864 2377216 18457088 144295936 1134209024 8952733696 70896263168 562816516096 4476410691584 35654557106176 284296086683648 2268726473261056 18115958466412544 144724547816390656
;; small program: loop(x + x, x, 1 + (loop((x + x) + x, x, 1) + loop(2 * (x + x), x, 1)))
;; fast program: loop(((x * x) * x) + x, 1, loop(x + x, x, 1)) + loop2(x * y, y, x, 1, 2 + (2 + 2))
(set-logic UFNIA)
...
(assert (forall ((x Int)) (= (h0 x) (+ 1 (+ (v1 x) (v2 x))))))
